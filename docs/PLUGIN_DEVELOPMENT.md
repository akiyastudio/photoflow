...
Use `component.media.v2` for files owned under `component.storage.v2`, and `project.progress.v2` to list/create image or video progress nodes and record generic source relationships. A progress create may include flat `sourceMetadata` (`category`, `role`, `displayName`, and `parentCapability`); each text value is at most 128 characters and `parentCapability` is `structural`, `workflow-input`, or `none`. The Host supplies the bound `componentId`, defaults omitted or empty metadata to structural progress, and rejects unknown/nested fields. Component-private media requests take only a relative path. Project media pages and variants include Host-managed external/virtual paths while preserving their virtual identity.

If `component.storage.v2` reports `adoption.state === "pending"`, no component storage path has been granted yet. Show a migration state, poll at 500–1000 ms with backoff, and reject all storage-backed reads and mutations until a committed response arrives. Do not create the advertised component directory, guess `dataPath`/`databasePath`, or start a second copy. After commit, validate the same-component receipt before rewriting private paths; keep project-output migration separately journaled and incremental. The Host retains the V1 source and recovers its copy journal after interruption.
