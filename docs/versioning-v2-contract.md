...
- 首屏版本关系使用只读 `progress_snapshot`；同一会话重开相同项目时可先复用最后一次成功的可信快照并立即刷新。目录身份、移动和缺失状态由 `progress_locations_snapshot` 在首屏之后协调；后台结果只能增量更新，不得先把已登记节点显示为“其他”。只读首屏查询与位置协调必须使用彼此隔离的工作进程，避免目录扫描或版本写入造成队头阻塞。
