...
Large first-time V1 adoptions are asynchronous. While the verified copy is running, `component.storage.v2` returns a discriminated `adoption.state:"pending"` result containing only the adoption identity and `startedAt`; `dataPath` and `databasePath` are intentionally absent and must not be inferred. Components may expose read status, but every storage read/write and mutation must fail closed. Poll status with bounded backoff (normally 500–1000 ms, not a tight loop). Once `state:"committed"` is returned, the response includes the component-owned paths plus the complete same-component receipt: adopted flags, legacy references, database digest, copied file count, and copied bytes. The Host keeps the V1 source for rollback and uses its adoption journal to discard or resume incomplete pending trees after a crash.
...
`project.progress.v2` supports `list`, `create`, and `relate`. It returns stable progress/edge IDs without folder paths. Create accepts a project virtual `relativePath`, `image`/`video` kind, version key, structural parent ID, optional generic `sourceProgressIds`, and optional `sourceMetadata`. Metadata is a flat allowlisted object: `category`, `role`, and `displayName` are non-empty control-character-free strings of at most 128 characters; `parentCapability` is `structural`, `workflow-input`, or `none`. The Host treats omitted or `{}` metadata as `{ category:'progress', parentCapability:'structural' }`, always overwrites `componentId` with the bound component identity, and rejects unknown/nested fields before creating the target directory. List returns persisted non-empty metadata; old empty records remain `null` so renderer compatibility/default inference continues to work. The versioning repository validates graph roles and cycles.
