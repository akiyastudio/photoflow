...
readline.createInterface({ input: process.stdin, crlfDelay: Infinity }).on('line', line => {
  ...
  send({ type: 'capability', id, parentId: frame.id, method: 'project.media.ratings.write.v1', payload: { idempotencyKey: String(frame.payload?.idempotencyKey || ''), items: frame.payload?.items } });
});
