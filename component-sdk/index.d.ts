...
export type ProjectMediaProcessRequest = { action: 'video.timelineFrames'; relativePath: string; times: number[] } | { action: 'video.trim'; idempotencyKey: string; relativePath: string; start: number; end: number; outputRelativePath: string; mode: 'fast' | 'exact' } | { action: 'office.extractImages'; idempotencyKey: string; relativePath: string; outputDirectory: string } | { action: 'video.sources.preview'; relativePaths: string[]; inputTokens: string[] } | { action: 'video.transcode.inspect'; relativePaths: string[]; inputTokens: string[]; settings: VideoToolSourceSettings } | { action: 'video.transcode'; idempotencyKey: string; relativePaths: string[]; inputTokens: string[]; settings: VideoToolSourceSettings; outputMode: 'new' | 'delete-original' } | { action: 'video.split'; idempotencyKey: string; relativePaths: string[]; inputTokens: string[] } | { action: 'status' | 'cancel' | 'pause' | 'resume'; idempotencyKey: string; processAction: 'video.trim' | 'office.extractImages' | 'video.transcode' | 'video.split' };
