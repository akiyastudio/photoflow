...
export type DialogsRequest = { kind: 'confirm'; title?: string; message?: string } | { kind: 'openFiles'; title?: string; extensions?: string[]; multiple?: boolean } | { kind: 'openDirectory'; title?: string; extensions?: string[]; recursive?: boolean; directoryToken?: boolean } | { kind: 'openOutput' | 'revealOutput'; commitId: string; artifactId: string };
export type ComponentAuthorizedInput = { name: string; relativeName: string; kind?: 'file' | 'directory'; token: string; expiresAt: number };
export type DialogsResponse = { apiVersion: 7; confirmed: boolean } | { apiVersion: 7; cancelled: boolean; inputs: ComponentAuthorizedInput[]; truncated?: boolean } | { apiVersion: 7; opened: true; outputRef: { commitId: string; artifactId: string } };
...
export type VideoToolSourceSettings = { container: 'mp4' | 'mov' | 'mkv'; videoMode: 'h264' | 'h265' | 'av1' | 'prores' | 'copy'; quality: 'high' | 'balanced' | 'small'; resolution: 'original' | '2160p' | '1080p' | '720p'; frameRate: 'original' | '24' | '25' | '30' | '50' | '60'; audioMode: 'copy' | 'aac' | 'remove'; subtitleMode: 'copy' | 'burn' | 'remove'; colorMode: 'auto' | 'sdr' | 'hdr10' | 'hlg' | 'hdr-to-sdr'; bitDepth: 'auto' | '8' | '10'; frameRateMode: 'preserve' | 'cfr' | 'vfr'; rotation: 'auto' | '0' | '90' | '180' | '270'; aspectMode: 'preserve' | 'square-pixels'; audioTrack: 'all' | 'first'; videoBitrateMbps: number | null; audioBitrateKbps: 96 | 128 | 160 | 192 | 256 | 320; encoderPreset: 'fast' | 'balanced' | 'quality' };
export type ProjectMediaProcessRequest = { action: 'video.timelineFrames'; relativePath: string; times: number[] } | { action: 'video.trim'; idempotencyKey: string; relativePath: string; start: number; end: number; outputRelativePath: string; mode: 'fast' | 'exact' } | { action: 'office.extractImages'; idempotencyKey: string; relativePath: string; outputDirectory: string } | { action: 'video.transcode.inspect'; relativePaths: string[]; inputTokens: string[]; settings: VideoToolSourceSettings } | { action: 'video.transcode'; idempotencyKey: string; relativePaths: string[]; inputTokens: string[]; settings: VideoToolSourceSettings; outputMode: 'new' | 'delete-original' } | { action: 'video.split'; idempotencyKey: string; relativePaths: string[]; inputTokens: string[] } | { action: 'status' | 'cancel' | 'pause' | 'resume'; idempotencyKey: string; processAction: 'video.trim' | 'office.extractImages' | 'video.transcode' | 'video.split' };
export type ProjectMediaProcessResponse = { apiVersion: 7; action: ProjectMediaProcessRequest['action']; receiptId?: string; operationId?: string; frames?: string[]; output?: { relativePath: string; size: number; sha256: string }; outputs?: Array<{ relativePath: string; size: number; sha256: string }>; mediaInfo?: JsonObject[]; capabilities?: JsonObject | null; estimatedOutputBytes?: number; report?: JsonObject[]; folderOutputs?: JsonObject[]; failedCount?: number; cancelled?: boolean; task?: { id: string; state: string; progress: number; message: string; checkpoint: JsonValue } | null };
