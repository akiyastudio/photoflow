...
export type ComponentLifecycleResponse = { componentId: string; componentVersion: string; permissions: ComponentPermission[]; events: VersionedName[]; lifecycleActions: string[]; state: 'active' } | { success: true; action: 'preflight' | 'install' | 'repair' | 'uninstall'; taskId: string; message: string };
...
export interface ProjectProgressManageResponse { action: ProjectProgressManageRequest['action']; receiptId: string; progress?: JsonObject; edge?: JsonObject; progressId?: string }
...
export type ProjectFilesMutateResponse = { planToken: string; expiresAt: number; action: string; undoCapability: 'available' | 'requires-precise-recycle'; items: JsonObject[] } | { receiptId: string; committed?: true; undone?: true; action?: string; undoAvailable?: boolean; undo?: JsonObject[] };
...
export type ComponentRuntimeExecuteResponse = { operationId?: string; result?: JsonObject; sourcePreviews?: JsonObject[]; cancelled?: boolean; task?: JsonObject | null };
