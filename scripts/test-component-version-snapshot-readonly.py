...
class ComponentVersionSnapshotTest(unittest.TestCase):
    def test_snapshot_is_bounded_scoped_and_side_effect_free(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            db.executescript("""
                CREATE TABLE projects(id TEXT PRIMARY KEY,name TEXT,status TEXT,is_deleted INTEGER NOT NULL DEFAULT 0);
            ...
            db.executemany("INSERT INTO projects VALUES(?,?,?,?)", [
                ("project-deleted", "Project", "active", 1),
                ("project-live", "Project", "active", 0),
                ("project-deleted-only", "Deleted Project", "active", 1),
            ])
            db.execute("INSERT INTO photos VALUES('photo-1','project-live',0)")
            db.execute("INSERT INTO photos VALUES('photo-deleted-project','project-deleted',0)")
            rows = [
                ...
                ("v-deleted-project", "photo-deleted-project", None, 1, "Deleted project", "original", os.path.join(scope_path, "deleted.jpg").casefold(), "ready", "", 1, 0, 0, 0, 30, 31, 0),
            ]
            ...
            with self.assertRaisesRegex(ValueError, "项目未登记"):
                media_versions_snapshot(db, {"projectName": "Deleted Project", "projectPath": project_path, "scopePath": scope_path, "limit": 10})
