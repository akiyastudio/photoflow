...
const run = async () => {
  try {
    ...
    const rebasePlan = []; await collectCopyPlan(rebaseSourceRoot, rebaseStagedRoot, rebasePlan); await copyPlannedFiles(rebasePlan, { ownershipToken: 'rebase-success' }); fs.renameSync(rebaseStagedRoot, rebaseProjectRoot);
    const rebased = await rebaseCleanupOwnership('rebase-success', rebaseStagedRoot, rebaseProjectRoot); assert.strictEqual(rebased.success, true);
    const rebasedCleanup = await removeCreatedPasteTargets([rebaseProjectRoot], { ownershipToken: 'rebase-success' }); assert.strictEqual(rebasedCleanup.success, true); assert.strictEqual(fs.existsSync(rebaseProjectRoot), false, 'a fully verified staged-to-project rebase remains cleanable bottom-up');
    const mismatchSourceRoot = path.join(root, 'rebase-mismatch-source'); const mismatchStagedRoot = path.join(root, 'rebase-mismatch-staged'); const mismatchProjectRoot = path.join(root, 'rebase-mismatch-project'); fs.mkdirSync(mismatchSourceRoot); fs.writeFileSync(path.join(mismatchSourceRoot, 'owned.txt'), 'owned-mismatch');
    const mismatchPlan = []; await collectCopyPlan(mismatchSourceRoot, mismatchStagedRoot, mismatchPlan); await copyPlannedFiles(mismatchPlan, { ownershipToken: 'rebase-mismatch' }); fs.renameSync(mismatchStagedRoot, mismatchProjectRoot); fs.writeFileSync(path.join(mismatchProjectRoot, 'owned.txt'), 'replacement-mismatch');
    const mismatchRebase = await rebaseCleanupOwnership('rebase-mismatch', mismatchStagedRoot, mismatchProjectRoot); assert.strictEqual(mismatchRebase.success, false); assert.strictEqual(fs.readFileSync(path.join(mismatchProjectRoot, 'owned.txt'), 'utf8'), 'replacement-mismatch', 'a rebase mismatch retains the complete recovery tree');
    const mismatchCleanup = await removeCreatedPasteTargets([mismatchProjectRoot], { ownershipToken: 'rebase-mismatch' }); assert.strictEqual(mismatchCleanup.success, false); assert.strictEqual(fs.existsSync(mismatchProjectRoot), true, 'a failed rebase never partially updates the path index to make the recovery tree destructively owned');

    const staleReleaseSource = path.join(root, 'rebase-stale-release-source'); const staleReleaseStaged = path.join(root, 'rebase-stale-release-staged'); const staleReleaseProject = path.join(root, 'rebase-stale-release-project'); fs.mkdirSync(staleReleaseSource); fs.writeFileSync(path.join(staleReleaseSource, 'owned.txt'), 'stale-release');
    const staleReleasePlan = []; await collectCopyPlan(staleReleaseSource, staleReleaseStaged, staleReleasePlan); await copyPlannedFiles(staleReleasePlan, { ownershipToken: 'rebase-stale-release' }); fs.renameSync(staleReleaseStaged, staleReleaseProject); let releasedDuringRebase = false;
    const staleReleaseRebase = await rebaseCleanupOwnership('rebase-stale-release', staleReleaseStaged, staleReleaseProject, { afterItemVerified: async () => { if (!releasedDuringRebase) { releasedDuringRebase = true; releaseCleanupOwnership('rebase-stale-release'); } } });
    assert.strictEqual(staleReleaseRebase.code, 'CLEANUP_REBASE_STALE'); const staleReleaseCleanup = await removeCreatedPasteTargets([staleReleaseProject], { ownershipToken: 'rebase-stale-release' }); assert.strictEqual(staleReleaseCleanup.success, false); assert.strictEqual(staleReleaseCleanup.outcomes.some(item => item.code === 'CLEANUP_UNOWNED_PATH'), true); assert.strictEqual(fs.existsSync(staleReleaseProject), true, 'a released snapshot cannot create ghost target-index ownership');
    ...
    const unknownBatchTree = await publishOwnedTree('root-unknown-batch', 'root-unknown-batch-owner');
    const unknownBatchNative = { ...nativePublication, compareDeleteFilesBatch: async requests => { await nativePublication.compareDeleteFilesBatch(requests); throw Object.assign(new Error('reply lost after destructive batch'), { code: 'FILE_PUBLICATION_PROTOCOL_ERROR' }); } };
    const unknownBatchCleanup = await removeCreatedPasteTargets([unknownBatchTree.project], { ownershipToken: 'root-unknown-batch-owner', nativePublicationService: unknownBatchNative }); assert.strictEqual(unknownBatchCleanup.success, false); assert.strictEqual(unknownBatchCleanup.outcomeUnknown, true); assert.strictEqual(unknownBatchCleanup.outcomes[0].partial, true); assert.strictEqual(unknownBatchCleanup.outcomes[0].partialUnknown, true); assert.strictEqual(unknownBatchCleanup.outcomes[0].recoveryPartial, true); assert.match(unknownBatchCleanup.outcomes[0].recoveryDescription, /仍可见的清理残留/);
    ...
    let crashBatchCalls = 0; const crashEntries = [fakeRootEntry, ...fakeDeleteEntries.slice(1, 130)];
    const crashedDeletion = await deleteOwnedTreeEntries(crashEntries, { compareDeleteFilesBatch: async requests => { crashBatchCalls += 1; if (crashBatchCalls === 1) return requests.map((_, index) => ({ index, success: true, deleted: true })); throw Object.assign(new Error('helper crashed after one unknown delete'), { code: 'FILE_PUBLICATION_PROTOCOL_ERROR' }); }, deleteDirectoriesBatch: async () => { throw new Error('ancestor deletion must stop after an unknown file batch'); } }, fakeRootEntry, manifestRoot, { yieldTreeCleanup: async () => undefined });
    assert.strictEqual(crashedDeletion.success, false); assert.strictEqual(crashedDeletion.deletedCount, 128, 'completed prior batches retain their confirmed delete count'); assert.strictEqual(crashedDeletion.outcomeUnknown, true); assert.strictEqual(crashedDeletion.partialUnknown, true); assert.strictEqual(crashedDeletion.outcomes.length, 1); assert.strictEqual(crashedDeletion.outcomes[0].success, false); assert.strictEqual(crashedDeletion.outcomes[0].outcomeUnknown, true, 'every item in a batch with no complete results is unknown rather than claimed retained');
