...
(async () => {
  const descriptor = { componentId: 'sample-component', componentVersion: '1', hostApiVersion: 2, service: { rpcMethods: ['sample.write.v1'], capabilities: [], permissions: [], protocolVersion: 1, runtime: 'node', entry: __filename } };
  let stopped = 0;
  let writtenFrame;
  const managed = {
    released: false,
