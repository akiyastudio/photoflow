...
const writeFixture = (id, mutate = () => undefined) => {
  ...
  const manifest = { apiVersion: 1, id, version: '1.0.0', displayName: `Fixture ${id}`, description: 'Generic development fixture', icon: 'ui/icon.svg', entrypoints: { default: 'algorithm.bin' }, requiredFiles: ['ui/index.html', 'ui/settings.html', 'ui/icon.svg', 'service.cjs'], componentHost: { contractVersion: 2, compatibility: { minHostApiVersion: 4, maxHostApiVersion: 4 }, contributions: [{ type: 'workspace.toolbarAction', id: 'open', label: 'Open', pageId: 'main' }, { type: 'component.fullPage', id: 'main', title: 'Fixture', entry: 'ui/index.html' }, { type: 'application.settingsPage', id: 'settings', label: 'Settings', entry: 'ui/settings.html', rpcMethods: [method] }], service: { protocolVersion: 1, runtime: 'node', entrypoints: { default: 'service.cjs' }, rpcMethods: [method], capabilities: ['component.settings.v2'], permissions: ['component.settings'], events: [], runtimeActions: [] } } };
...
try {
  ...
  for (const [id, pattern] of [['missing-build', /missing or unsafe/], ['path-escape', /component-local relative file/], ['unknown-field', /Unknown component development field/], ['undeclared-map', /undeclared component file/], ['permission-default-deny', /permissions allowlist/]]) {
