...
for (const section of [hook.slice(hook.indexOf('const showNotice'), hook.indexOf('const showComponentNotification')), hook.slice(hook.indexOf('const showComponentNotification'), hook.indexOf('useEffect(() => {'))]) assert(section.indexOf('timersRef.current.set') < section.indexOf('enqueueNotice'), 'timers must be registered before enqueue so self-eviction clears them');
