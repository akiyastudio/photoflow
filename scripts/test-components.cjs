...
assert(teamRetouchManager.includes("cropEditor ? 'overflow-visible' : 'overflow-hidden'"), '范围编辑弹窗打开时不应被图片卡片裁切');
  ...
  assert(teamRetouchSteps.includes('人物识别') && teamRetouchSteps.includes('标记人物') && teamRetouchSteps.includes('工作流程'), 'team retouch must expose one shared three-step workflow');
