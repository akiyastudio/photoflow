...
assert(teamRetouchManager.includes('aria-label={`选择人物身份“${identity.name}”`}') && teamRetouchManager.includes('className="absolute inset-0 z-10 cursor-pointer'), 'existing identity cards must use a real full-card button so clicking the image or label always selects the person');
...
assert(teamRetouchManager.includes('uniqueIdentitySubjectsPerPhoto'), 'identity candidate selection must prevent duplicate assignments from the same photo');
  ...
  assert(personIdentityManager.includes('startWorkflowIdentityPointerDrag') && personIdentityManager.includes('onPointerCancel') && personIdentityManager.includes('data-workflow-identity-id') && personIdentityManager.includes('moveWorkflowIdentityWithKeyboard'), 'workflow ordering must provide a dedicated pointer/touch drag handle with keyboard fallback');
  ...
  assert(personIdentityManager.includes('const predecessorReturn = [...predecessors].reverse().map(completedReturnPath).find(Boolean)') && personIdentityManager.includes('sourcePath={item.workflowImagePath}'), 'workflow thumbnails must display the same predecessor return that is handed to the current retoucher');
