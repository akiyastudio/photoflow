...
assert(teamRetouchManager.includes('uniqueIdentitySubjectsPerPhoto'), 'identity candidate selection must prevent duplicate assignments from the same photo');
  ...
  assert(projectWorkspace.includes('在新标签页打开') && projectWorkspace.includes('onOpenDirectoryPage(entry.relativePath)') && projectWorkspace.includes("!fileMenu.entry.viaShortcut"), 'project and inspiration folders must open independent tabs without granting that action to shortcut content');
