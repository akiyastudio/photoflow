...
function validateFfmpegManifest(manifest, artifactRoot) {
  if (manifest.schemaVersion !== 1 || manifest.kind !== 'photoflow-ffmpeg-runtime') throw new Error('FFmpeg 运行时清单格式无效');
...
function validateMpvManifest(manifest, artifactRoot) {
  if (manifest.schemaVersion !== 1 || manifest.kind !== 'photoflow-libmpv-runtime') throw new Error('libmpv 运行时清单格式无效');
  ...
  if (ffmpegFlags.some(flag => flag === '--enable-gpl' || flag === '--enable-nonfree' || flag === '--enable-libx264' || FORBIDDEN_FFMPEG_FLAGS.includes(flag))) {
