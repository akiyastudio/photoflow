...
assert(fileTransferToast.includes('Minus') && fileTransferToast.includes('aria-label="缩小到后台"') && !fileTransferToast.includes('minimizeTaskToast(task.id)} className="shrink-0 rounded-md px-2.5 py-1.5 text-xs font-bold text-red'), 'minimizing a file-transfer toast must use its own control rather than the cancel action');
