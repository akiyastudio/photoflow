...
(async () => {
  ...
  await React.act(async () => {
    ...
    assert.strictEqual(await queuedConflictUndo, true, 'undo queued before stale recovery must execute after the recovered drag save');
    await Promise.resolve(); await Promise.resolve(); await Promise.resolve(); await Promise.resolve();
  });
  assert.strictEqual(layoutRequests.saves.length, savesBeforeStaleRetry + 3, 'stale layout saves must reload and retry once before executing the already queued undo');
  ...
  const rawBeforeQueuedFailure = { left: rawCanvasNode.style.left, top: rawCanvasNode.style.top };
  const freeBeforeQueuedFailure = { left: freeCanvasNode.style.left, top: freeCanvasNode.style.top };
  layoutRequests.holdSaves = true;
  layoutRequests.failNextSave = true;
  await React.act(async () => {
    ...
    await Promise.resolve(); await Promise.resolve();
    dispatch(freeCanvasNode, 'pointerdown', { pointerId: 432, button: 0, clientX: 300, clientY: 300 });
    dispatch(freeCanvasNode, 'pointermove', { pointerId: 432, button: 0, clientX: 340, clientY: 340 });
    dispatch(freeCanvasNode, 'pointerup', { pointerId: 432, button: 0, clientX: 340, clientY: 340 });
  });
  ...
  assert.strictEqual(layoutRequests.saves.length, savesBeforeQueuedFailure + 2, 'a failed layout write must not swallow the later command already queued by the user');
  assert.deepStrictEqual({ left: rawCanvasNode.style.left, top: rawCanvasNode.style.top }, rawBeforeQueuedFailure, 'a failed drag must roll back its node even when a different-node command is queued later');
  assert.notDeepStrictEqual({ left: freeCanvasNode.style.left, top: freeCanvasNode.style.top }, freeBeforeQueuedFailure, 'the later different-node drag must remain visible');
  assert.strictEqual(parseFloat(freeCanvasNode.style.left), layoutRequests.saves.at(-1).positions.find(position => position.nodeKey === 'progress:free').x + 32, 'the later different-node drag must remain identical in the UI and persisted layout');
  ...
  const rawBeforeQueuedRefreshFailure = { left: rawCanvasNode.style.left, top: rawCanvasNode.style.top };
  const savesBeforeQueuedRefreshFailure = layoutRequests.saves.length;
  layoutRequests.holdSaves = true;
  layoutRequests.failNextSave = true;
  let queuedRefreshFailure;
  await React.act(async () => {
    queuedRefreshFailure = canvasController.refreshLayout();
    await Promise.resolve(); await Promise.resolve();
    dispatch(freeCanvasNode, 'pointerdown', { pointerId: 435, button: 0, clientX: 300, clientY: 300 });
    dispatch(freeCanvasNode, 'pointermove', { pointerId: 435, button: 0, clientX: 360, clientY: 340 });
    dispatch(freeCanvasNode, 'pointerup', { pointerId: 435, button: 0, clientX: 360, clientY: 340 });
    layoutRequests.holdSaves = false;
    layoutRequests.saveReleases.splice(0).forEach(release => release());
    assert.strictEqual(await queuedRefreshFailure, false);
    await new Promise(resolve => setImmediate(resolve));
  });
  assert.deepStrictEqual(layoutRequests.saves.slice(savesBeforeQueuedRefreshFailure).map(request => request.mode), ['replace', 'patch'], 'a drag queued after a failed refresh must still persist in order');
  assert.deepStrictEqual({ left: rawCanvasNode.style.left, top: rawCanvasNode.style.top }, rawBeforeQueuedRefreshFailure, 'failed refresh must roll back untouched nodes even when a later local drag exists');
  const queuedRefreshPatch = layoutRequests.saves.at(-1).positions.find(position => position.nodeKey === 'progress:free');
  assert.strictEqual(parseFloat(freeCanvasNode.style.left), queuedRefreshPatch.x + 32, 'failed refresh rollback must preserve the later modified node in the UI');
  assert.deepStrictEqual(layoutRequests.positions.find(position => position.nodeKey === 'progress:free'), queuedRefreshPatch, 'failed refresh followed by a drag must leave UI and server coordinates identical');
  ...
  await React.act(async () => {
    dispatch(refreshRaceRawNode, 'pointerdown', { pointerId: 433, button: 0, clientX: 100, clientY: 100 });
    dispatch(refreshRaceRawNode, 'pointermove', { pointerId: 433, button: 0, clientX: 300, clientY: 240 });
    dispatch(refreshRaceRawNode, 'pointerup', { pointerId: 433, button: 0, clientX: 300, clientY: 240 });
    await Promise.resolve(); await Promise.resolve();
  });
  const manualBeforeRefreshRace = { left: refreshRaceRawNode.style.left, top: refreshRaceRawNode.style.top };
  assert.notDeepStrictEqual(manualBeforeRefreshRace, defaultBeforeRefreshRace, 'refresh race setup must first persist a visibly manual coordinate');
  const savesBeforeRefreshRace = layoutRequests.saves.length;
  layoutRequests.holdSaves = true;
  let pendingRefreshRace;
  await React.act(async () => {
    pendingRefreshRace = canvasController.refreshLayout();
    await Promise.resolve(); await Promise.resolve();
    dispatch(refreshRaceRawNode, 'pointerdown', { pointerId: 434, button: 0, clientX: 100, clientY: 100 });
    dispatch(refreshRaceRawNode, 'pointermove', { pointerId: 434, button: 0, clientX: 160, clientY: 140 });
    dispatch(refreshRaceRawNode, 'pointerup', { pointerId: 434, button: 0, clientX: 160, clientY: 140 });
    layoutRequests.holdSaves = false;
    layoutRequests.saveReleases.splice(0).forEach(release => release());
    assert.strictEqual(await pendingRefreshRace, true);
    await new Promise(resolve => setImmediate(resolve));
  });
  assert.deepStrictEqual(layoutRequests.saves.slice(savesBeforeRefreshRace).map(request => request.mode), ['replace', 'patch'], 'refresh and a later drag must persist in one deterministic order');
  const refreshRacePatch = layoutRequests.saves.at(-1).positions.find(position => position.nodeKey === 'progress:raw');
  assert.strictEqual(parseFloat(refreshRaceRawNode.style.left), refreshRacePatch.x + 32, 'a completed refresh must not overwrite the later drag in the UI');
  assert.deepStrictEqual(layoutRequests.positions.find(position => position.nodeKey === 'progress:raw'), refreshRacePatch, 'the final server layout must match the last visible drag');
  ...
  const loadsBeforeOrdinaryActiveFailure = layoutRequests.loads;
  layoutRequests.holdSaves = true;
  layoutRequests.failAtSave = savesBeforeOrdinaryActiveFailure + 1;
  layoutRequests.failAtLoad = loadsBeforeOrdinaryActiveFailure + 1;
  await React.act(async () => {
  ...
  assert.strictEqual(layoutRequests.loads, loadsBeforeOrdinaryActiveFailure + 1, 'ordinary failed A1 must exercise exactly one deliberately failed compensation load');
  ...
  const loadsBeforeStaleActiveFailure = layoutRequests.loads;
  ...
  layoutRequests.failAtLoad = loadsBeforeStaleActiveFailure + 2;
  await React.act(async () => {
  ...
  assert.strictEqual(layoutRequests.loads, loadsBeforeStaleActiveFailure + 2, 'stale recovery must use one successful latest fetch followed by one deliberately failed compensation load');
