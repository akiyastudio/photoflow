...
const testPrepareSidecars = async items => items.map(item => { const stat = fs.lstatSync(item.path); return { path: item.path, role: item.role, size: item.expectedContent.length, sha256: crypto.createHash('sha256').update(item.expectedContent).digest('hex'), nativeIdentity: `${stat.dev}:${stat.ino}` }; });
...
const testVerifySidecar = async ({ path: sidecarPath, size, sha256, nativeIdentity }) => { const stat = fs.lstatSync(sidecarPath); const value = fs.readFileSync(sidecarPath); assert.equal(`${stat.dev}:${stat.ino}`, nativeIdentity); assert.equal(value.length, size); assert.equal(crypto.createHash('sha256').update(value).digest('hex'), sha256); return true; };
const finalizeComponentCleanupProof = (receipt, options = {}) => finalizeComponentCleanupProofRaw(receipt, { ...options, deleteSidecar: testDeleteSidecar, verifySidecar: testVerifySidecar });
...
(async () => {
  try {
    ...
    const orchestratedRoot = path.join(root, 'orchestrated-cleanup'); fs.mkdirSync(orchestratedRoot); fs.writeFileSync(path.join(orchestratedRoot, 'a'), 'a'); const orchestratedReceipt = await receiptForDirectory(orchestratedRoot); const phases = []; const orchestrator = createComponentCleanupOrchestrator({ captureNativeProof: testCaptureNativeProof, prepareSidecars: testPrepareSidecars, persistPrepared: async () => true, deleteOwned: testDeleteOwned, deleteSidecar: testDeleteSidecar, verifySidecar: testVerifySidecar, persistPhase: async phase => { phases.push(phase); return true; } }); const orchestrated = await orchestrator.run(orchestratedReceipt); assert.equal(orchestrated.status, 'complete'); assert.deepEqual(phases, ['pending', 'data-complete']); assert.equal((await orchestrator.finalize(orchestrated.receipt)).status, 'pending'); assert.equal((await orchestrator.finalize(orchestrated.receipt, { completionFlushed: true })).status, 'complete');
    let forgedSidecarDeleteCalls = 0; const rejectPrepared = value => assert.rejects(finalizeComponentCleanupProofRaw(value, { dataCleanupCompletePersisted: true, verifySidecar: testVerifySidecar, deleteSidecar: async () => { forgedSidecarDeleteCalls += 1; } }), error => error.code === 'COMPONENT_CLEANUP_PREPARED_INVALID'); const validPreparedItems = orchestrated.receipt.sidecarReceipts;
    await rejectPrepared({ ...orchestrated.receipt, sidecarReceipts: [...validPreparedItems, { ...validPreparedItems[0], path: path.join(root, 'arbitrary') }] });
    await rejectPrepared({ ...orchestrated.receipt, sidecarReceipts: [validPreparedItems[0], validPreparedItems[0], validPreparedItems[2]] });
    await rejectPrepared({ ...orchestrated.receipt, sidecarReceipts: validPreparedItems.filter(item => item.role !== 'proof') });
    await rejectPrepared({ ...orchestrated.receipt, sidecarReceipts: Array.from({ length: 32 }, () => validPreparedItems[0]) }); assert.equal(forgedSidecarDeleteCalls, 0, 'invalid prepared metadata is rejected before native deletion');
    const semanticPreflightRoot = path.join(root, 'semantic-preflight'); fs.mkdirSync(semanticPreflightRoot); fs.writeFileSync(path.join(semanticPreflightRoot, 'a'), 'a'); const semanticReceipt = await receiptForDirectory(semanticPreflightRoot); await cleanupOwnedComponentPath(semanticReceipt); for (const [role, field, value] of [['intent', 'sha256', '0'.repeat(64)], ['intent', 'size', 1], ['intent', 'nativeIdentity', 'wrong-native'], ['proof', 'nativeIdentity', 'wrong-native'], ['verified', 'nativeIdentity', 'wrong-native']]) { const forged = { ...semanticReceipt, sidecarReceipts: semanticReceipt.sidecarReceipts.map(item => item.role === role ? { ...item, [field]: value } : { ...item }) }; let semanticDeleteCalls = 0; await assert.rejects(finalizeComponentCleanupProofRaw(forged, { dataCleanupCompletePersisted: true, verifySidecar: testVerifySidecar, deleteSidecar: async () => { semanticDeleteCalls += 1; } })); assert.equal(semanticDeleteCalls, 0, `${role}.${field} must fail complete preflight before deletion`); }
