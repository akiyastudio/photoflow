...
(async () => {
  try {
    ...
    let forgedSidecarDeleteCalls = 0; const rejectPrepared = value => assert.rejects(finalizeComponentCleanupProofRaw(value, { dataCleanupCompletePersisted: true, deleteSidecar: async () => { forgedSidecarDeleteCalls += 1; } }), error => error.code === 'COMPONENT_CLEANUP_PREPARED_INVALID'); const validPreparedItems = orchestrated.receipt.sidecarReceipts;
