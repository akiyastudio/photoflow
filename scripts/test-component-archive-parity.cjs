...
const { MAX_ARCHIVE_BYTES, MAX_ENTRY_BYTES, MAX_PACKAGE_BYTES, captureVerifiedComponentTreeIdentity, cleanupOwnedComponentPath: cleanupOwnedComponentPathImplementation, componentSubtreeIdentity, componentTreeIdentityReceipt, extractComponentArchive, inspectComponentArchive, reserveComponentInstallCapacity, snapshotComponentArchive, validateComponentTreeIdentityReceipt } = require('../electron/component-package-archive.cjs');
...
const cleanupOwnedComponentPath = receipt => cleanupOwnedComponentPathImplementation(receipt, { root, allowMissing: true, allowPartial: true });
...
(async () => {
  try {
    ...
    await cleanupOwnedComponentPath(rewrittenCleanupReceipt);
    ...
    let retainedReceipt;
    await assert.rejects(extractComponentArchive(failedInspection, path.join(root, 'failed-extraction')), error => {
      retainedReceipt = error.cleanupPendingReceipts?.[0];
      return retainedReceipt?.kind === 'directory' && Array.isArray(retainedReceipt.treeIdentity) && fs.existsSync(retainedReceipt.path);
    });
    await cleanupOwnedComponentPath(retainedReceipt);
    assert.equal(fs.existsSync(retainedReceipt.path), false, 'failed extraction returns an original receipt for durable host cleanup');
