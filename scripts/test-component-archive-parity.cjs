...
(async () => {
  try {
    ...
    await assert.rejects(cleanupOwnedComponentPathRaw(finalSwapDirectoryReceipt, { captureNativeProof: testCaptureNativeProof, deleteOwned: async ({ isolatedPath, proof }) => { assert.equal(proof.native.rootIdentity, `${finalSwapDirectoryReceipt.nodeIdentity.dev}:${finalSwapDirectoryReceipt.nodeIdentity.ino}`); await originalRename(isolatedPath, finalSwapDirectoryOwned); fs.mkdirSync(isolatedPath); fs.writeFileSync(path.join(isolatedPath, 'replacement'), 'replacement'); throw new Error('bound recursive delete rejected swapped final path'); } }), /bound recursive|身份|新增或变化节点/);
