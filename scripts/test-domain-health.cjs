...
const titlebarSource = fs.readFileSync(path.join(root, 'src', 'features', 'app', 'AppTitlebar.tsx'), 'utf8');
...
assert(appSource.includes('<AppTitlebar') && appSource.includes('components={components}') && titlebarSource.includes('<DomainHealthBanner components={props.components}') && bannerSource.includes('getDomainHealth') && bannerSource.includes('部分功能已隔离') && bannerSource.includes('retryDomainCommand'), 'renderer must pass component health into the titlebar and expose domain degradation and dead-letter recovery');
