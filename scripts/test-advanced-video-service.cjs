...
const { readPeDependencies } = require('../extensions/video-playback-mpv/scripts/vendor/pe-dependency-closure.cjs');
...
const run = async () => {
  ...
  const mediaPreviewSource = require('fs').readFileSync(path.join(__dirname, '..', 'src', 'features', 'workspace', 'MediaPreviewPane.tsx'), 'utf8');
  ...
  assert(playerSource.includes('topRightOverlayHole') && !playerSource.includes('marginTop: Math.max(0, topOverlayInset)') && mediaPreviewSource.includes('topRightOverlayHole={fullscreen && fullscreenControlsVisible ? 60 : 0}'), 'full-screen video must reach the top edge while keeping the close button interactive without exposing a rectangular patch');
  assert(playerSource.includes('controlsOverlay ? <div ref={controlsOverlayRef} className="absolute inset-x-0 bottom-0') && mediaPreviewSource.includes('controlsVisible={!fullscreen || fullscreenControlsVisible}') && mediaPreviewSource.includes('controlsOverlay={fullscreen}'), 'full-screen playback controls must float over a full-size video surface, hide with the close button, and return on pointer activity');
  assert(mediaPreviewSource.includes('FULLSCREEN_CONTROLS_HIDE_DELAY_MS = 1800'), 'full-screen media controls must hide after exactly 1.8 seconds of inactivity');
  assert(mediaPreviewSource.includes('projectWorkspaceClient.setWindowFullscreen(true)') && mediaPreviewSource.includes('projectWorkspaceClient.setWindowFullscreen(false)') && systemIpcSource.includes("setAlwaysOnTop(true, 'screen-saver', 1)") && systemIpcSource.includes('targetWindow.setKiosk(true)') && systemIpcSource.includes('targetWindow.focus()'), 'media preview full-screen must focus a Windows kiosk window above the taskbar without requiring a follow-up click');
