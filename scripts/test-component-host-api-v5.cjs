...
const capabilities = ['project.files.page.v1', 'project.files.search.v1', 'project.media.metadata.v1', 'project.versions.page.v1', 'project.version.graph.v1', 'project.media.ratings.v1'];
...
(async () => {
  try {
    const first = await broker.invoke(descriptor, 'project.files.page.v1', { pageSize: 2 }, context);
    ...
    const second = await broker.invoke(descriptor, 'project.files.page.v1', { pageSize: 2, cursor: first.page.cursor }, context);
    assert(second.items.some(item => item.kind === 'sidecar')); assert(second.items.every(item => !('path' in item)));
    const search = await broker.invoke(descriptor, 'project.files.search.v1', { query: 'notes', pageSize: 10 }, context);
    assert.deepEqual(search.items.map(item => item.relativePath), ['notes.txt']);
    const metadata = await broker.invoke(descriptor, 'project.media.metadata.v1', { relativePath: 'photo.jpg' }, context);
    ...
    const videoMetadata = await broker.invoke(descriptor, 'project.media.metadata.v1', { relativePath: 'nested/clip.mp4' }, context);
    assert.equal(videoMetadata.video.codec, 'h264'); assert.equal(videoMetadata.video.durationSeconds, 12.5);
    const missingMetadata = await broker.invoke(descriptor, 'project.media.metadata.v1', { relativePath: 'missing-metadata.jpg' }, context);
    ...
    const versions = await broker.invoke(descriptor, 'project.versions.page.v1', { pageSize: 10 }, context);
    ...
    const versionFirst = await broker.invoke(descriptor, 'project.versions.page.v1', { pageSize: 1 }, context);
    const versionSecond = await broker.invoke(descriptor, 'project.versions.page.v1', { pageSize: 1, cursor: versionFirst.page.cursor }, context);
    assert.equal(versionSecond.page.truncated, true, 'truncated survives cursor continuation');
    versionSnapshotTruncated = false;
    const graph = await broker.invoke(descriptor, 'project.version.graph.v1', {}, context);
    ...
    const scopedGraph = await broker.invoke(descriptor, 'project.version.graph.v1', {}, { ...context, scopeRelativePath: 'nested' });
    ...
    const missingGraph = await broker.invoke(descriptor, 'project.version.graph.v1', { includeMissing: true }, { ...context, scopeRelativePath: 'nested' });
    ...
    const starvationGraph = await broker.invoke(descriptor, 'project.version.graph.v1', {}, { ...context, scopeRelativePath: 'nested' });
    ...
    const visibleOverflowGraph = await broker.invoke(descriptor, 'project.version.graph.v1', {}, { ...context, scopeRelativePath: 'nested' });
    ...
    const scanOverflowGraph = await broker.invoke(descriptor, 'project.version.graph.v1', {}, { ...context, scopeRelativePath: 'nested' });
    ...
    const ratings = await broker.invoke(descriptor, 'project.media.ratings.v1', { mediaRefs: [{ relativePath: 'photo.jpg' }, { relativePath: 'nested/clip.mp4' }] }, context);
    ...
    for (const invalid of [{ pageSize: 0 }, { pageSize: 1.5 }, { pageSize: '2' }, { pageSize: 201 }, { cursor: 'x' }, { cursor: 'x'.repeat(81) }, { unknown: true }]) await assert.rejects(broker.invoke(descriptor, 'project.files.page.v1', invalid, context), error => error.code === 'COMPONENT_HOST_INVALID_REQUEST');
    await assert.rejects(broker.invoke(descriptor, 'project.files.search.v1', { pageSize: 1 }, context), error => error.code === 'COMPONENT_HOST_INVALID_REQUEST');
    const mismatchPage = await broker.invoke(descriptor, 'project.files.page.v1', { pageSize: 1 }, context);
    await assert.rejects(broker.invoke(descriptor, 'project.files.page.v1', { pageSize: 2, cursor: mismatchPage.page.cursor }, context), error => error.code === 'COMPONENT_HOST_INVALID_REQUEST');
    const searchCursor = await broker.invoke(descriptor, 'project.files.search.v1', { query: 'o', pageSize: 1 }, context);
    await assert.rejects(broker.invoke(descriptor, 'project.files.search.v1', { query: 'other', pageSize: 1, cursor: searchCursor.page.cursor }, context), error => error.code === 'COMPONENT_HOST_INVALID_REQUEST');
    await assert.rejects(broker.invoke(descriptor, 'project.media.metadata.v1', { relativePath: 'photo.jpg', extra: true }, context), error => error.code === 'COMPONENT_HOST_INVALID_REQUEST');
    await assert.rejects(broker.invoke(descriptor, 'project.version.graph.v1', { includeMissing: 'yes' }, context), error => error.code === 'COMPONENT_HOST_INVALID_REQUEST');
    await assert.rejects(broker.invoke(descriptor, 'project.version.graph.v1', { extra: true }, context), error => error.code === 'COMPONENT_HOST_INVALID_REQUEST');
    await assert.rejects(broker.invoke(descriptor, 'project.versions.page.v1', { extra: true }, context), error => error.code === 'COMPONENT_HOST_INVALID_REQUEST');
    await assert.rejects(broker.invoke(descriptor, 'project.media.ratings.v1', { mediaRefs: [{ relativePath: 'photo.jpg' }, { relativePath: 'photo.jpg' }] }, context), error => error.code === 'COMPONENT_HOST_INVALID_REQUEST');
    await assert.rejects(broker.invoke(descriptor, 'project.media.ratings.v1', { mediaRefs: [{ relativePath: 'photo.jpg', extra: true }] }, context), error => error.code === 'COMPONENT_HOST_INVALID_REQUEST');
    await assert.rejects(broker.invoke(descriptor, 'project.media.ratings.v1', { mediaRefs: ['photo.jpg'] }, context), error => error.code === 'COMPONENT_HOST_INVALID_REQUEST');
    await assert.rejects(broker.invoke(descriptor, 'project.media.metadata.v1', { relativePath: '../secret.jpg' }, context), error => error.code === 'COMPONENT_HOST_INVALID_REQUEST');
    const scoped = { ...context, scopeRelativePath: 'nested' };
    await assert.rejects(broker.invoke(descriptor, 'project.media.metadata.v1', { relativePath: 'photo.jpg' }, scoped), error => error.code === 'COMPONENT_HOST_PERMISSION_DENIED');
    const otherDescriptor = { ...descriptor, componentId: 'other', service: descriptor.service };
    const cursorPage = await broker.invoke(descriptor, 'project.files.page.v1', { pageSize: 1 }, context);
    await assert.rejects(broker.invoke(otherDescriptor, 'project.files.page.v1', { cursor: cursorPage.page.cursor }, context), error => error.code === 'COMPONENT_HOST_TOKEN_EXPIRED');
    ...
    await assert.rejects(broker.invoke(descriptor, 'project.files.page.v1', { pageSize: 1, cursor: cursorPage.page.cursor }, { ...context, workspacePath: restoredWorkspace }), error => error.code === 'COMPONENT_HOST_TOKEN_EXPIRED');
    assert.throws(() => broker.invoke({ ...descriptor, service: { ...descriptor.service, capabilities: descriptor.service.capabilities.filter(item => item !== 'project.files.page.v1') } }, 'project.files.page.v1', {}, context), /not granted/);
    ...
    try {
      ...
      await assert.rejects(broker.invoke(descriptor, 'project.files.page.v1', {}, { ...context, scopeRelativePath: 'linked-scope' }), error => error.code === 'COMPONENT_HOST_PERMISSION_DENIED');
      ...
      await assert.rejects(broker.invoke(descriptor, 'project.files.page.v1', {}, context), error => error.code === 'COMPONENT_HOST_PERMISSION_DENIED');
