...
assert(previewNavigationSource.includes('setPreviewPath(nextEntry.relativePath)') && previewNavigationSource.includes('setPreviewMediaPath(nextEntry.relativePath)') && !previewNavigationSource.includes('setSelectedPaths') && !previewNavigationSource.includes('selectionAnchorPathRef'), 'keyboard preview navigation must move only the preview cursor and preserve the complete file selection');
...
assert((projectWorkspace.match(/project-file-entry-preview/g) || []).length >= 4 && indexCss.includes('.project-file-entry-preview') && !previewCursorCss.includes('outline:') && indexCss.includes('background:transparent !important') && indexCss.includes('box-shadow:none !important') && indexCss.includes('.project-file-entry-preview .file-grid-select') && indexCss.includes('.project-file-entry-preview .file-select-box { visibility:hidden; }') && projectWorkspace.includes("querySelectorAll<HTMLElement>('[data-entry-path]')") && projectWorkspace.includes('entryNode.focus({ preventScroll: true })'), 'grid, list, recursive, and version-tree previews must use the single native focus outline without selected fill, ring, checkbox, or a second custom outline');
...
assert(workspaceIpc.includes("virtualPaths.resolve(root, normalizedRelativePath, { externalRootMode: 'target' })") && workspaceIpc.includes('virtualPaths.readManagedExternalLink(entryPath)') && projectWorkspace.includes('if (isFolderLikeEntry(entry) && !external)') && projectWorkspace.includes('isFolderLikeEntry(entry) && entry.externalLink && !range') && projectWorkspace.includes("entry.externalLink ? '外链'") && projectWorkspace.includes('entry.externalLink && <span aria-label="外链" className="shortcut-cover-badge"') && projectWorkspace.includes('isFolderLikeEntry(fileMenu.entry)') && projectWorkspace.includes('标记…'), 'managed external folders must open in-app on a plain single click, browse through the trusted virtual-path service, use the normal folder layout with an independently overlaid external-link badge, and remain eligible for unified purpose marking');
