...
assert(/导入文件默认行为[\s\S]*?视频分割、视频预览和 RAW 转 JPG 同时用于“从 SD 卡导入”和“导入底片”[\s\S]*?超过 4GB 的视频自动分割[\s\S]*?生成视频预览[\s\S]*?预览质量[\s\S]*?导入 RAW 文件时[\s\S]*?导入后默认删除源文件/.test(settingsFeature), 'shared import defaults must be presented in one group with their SD-card and negative-import scope explained');
...
assert(toolViews.includes('if (croppedRelativePaths.length) await onFilesChanged?.()') && projectWorkspace.includes('directoryEntriesCacheRef.current.clear()') && projectWorkspace.includes('setRecentRefreshToken(value => value + 1)') && projectWorkspace.includes('await refresh(currentRelativePathRef.current)'), 'screenshot main-image completion must immediately refresh the active file list and recursive results');
...
assert(!marqueeStartSource.includes('setPreviewPaneOpen(false)') && !marqueeStartSource.includes('setMetadataPaneOpen(false)') && marqueeFinishSource.includes('clearPreviewAfterSelectionDrag(drag)') && projectWorkspace.includes('Math.min(surfaceRect.top, drag.startY)') && projectWorkspace.includes('Math.max(surfaceRect.bottom, drag.startY)'), 'marquee selection must keep the file-column layout and drag origin stable until pointer release');
...
assert(protectedProjectFolderService.includes("'raw'") && protectedProjectFolderService.includes("'mov_预览'") && protectedProjectFolderService.includes("'图片选片'") && protectedProjectFolderService.includes("'策划'") && protectedProjectFolderService.includes("'团片协作'") && projectWorkspace.includes("'视频选片', '策划', '团片协作'") && filesIpc.includes('isProtectedProjectFolderPath'), 'workflow-managed root folders must reject ordinary rename operations in both renderer and backend');
