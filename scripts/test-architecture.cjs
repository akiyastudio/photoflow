...
assert(settingsFeature.includes('启动时自动从 SD 卡导入') && settingsFeature.includes('默认关闭') && app.includes('autoStart: false'), 'startup SD import must be clearly named and default off');
assert(toolViews.includes('autoCheckStartedRef') && toolViews.includes('!autoCheckStartedRef.current') && toolViews.includes('autoCheckStartedRef.current = true'), 'startup SD import checks must run once under React strict mode');
...
assert(app.includes('openProjectDirectoryPage') && app.includes('currentRelativePath: initialRelativePath, initialRelativePath') && projectWorkspace.includes('onOpenDirectoryPage?.(entry.relativePath)'), 'opening a project folder in a new page must create a distinct mounted workspace at that initial path');
...
assert(settingsFeature.includes('<option value="medium">中（默认 · 约 4 Mbps）</option>') && settingsFeature.includes('<option value="high">高（约 10 Mbps）</option>') && !settingsFeature.includes('<option value="maximum">'), 'import settings must expose medium and high video preview qualities in the unified row control');
...
assert(workspaceTabModel.includes("page.currentRelativePath === ''") && !workspaceTabModel.includes("page.initialRelativePath === ''") && workspaceTabModel.includes('selectProjectFromSidebar') && workspaceTabsHook.includes('selectSidebarProject'), 'sidebar project selection must reuse a page currently at the root or create one without collapsing child pages');
...
assert(/title="导入"[\s\S]*?title="导入后默认删除源文件"[\s\S]*?title="导入底片"[\s\S]*?title="自动分割大型视频"[\s\S]*?title="生成视频预览"[\s\S]*?title="视频预览质量"[\s\S]*?title="RAW 缺少同名 JPG 时自动创建"/.test(settingsFeature), 'the all-import source deletion default must have its own group before negative-import settings');
assert(/title="导入"[\s\S]*?title="导入底片"[\s\S]*?title="从 SD 卡导入"[\s\S]*?title="已记录的 SD 卡设备"[\s\S]*?title="花絮导入"/.test(settingsFeature), 'import settings must use the requested general, negative, SD-card, device-history, and B-roll order');
...
assert(toolViews.includes("const queue = directSource") && /if \(type === 'work' && config\?\.generateVideoPreview\) args\.push\('--generate_video_preview', '--video_preview_quality', config\.videoPreviewQuality\)/.test(toolViews) && toolViews.includes("if (type === 'work' && config?.splitLargeFiles) args.push('--split_large_files')"), 'negative imports must share the large-file splitting, video preview generation, and quality settings');
assert(toolViews.includes("if (type === 'broll' && splitLargeBrollFiles) args.push('--split_large_files')") && classifyImport.includes('args.delete_source, args.split_large_files') && classifyImport.includes('def split_broll_video(input_path, keep_original=False)'), 'SD-card B-roll imports must use the existing large-video split setting and cancellable splitter');
assert(classifyImport.includes('def promote_staged_file') && classifyImport.includes('os.replace(source, destination)') && classifyImport.includes('allow_atomic_move=True') && ffmpegTranscode.includes('f".photoflow-split-') && classifyImport.includes('split_broll_video(destination, keep_original=True)'), 'B-roll imports must atomically promote same-volume staged files and transactionally split them without a second full copy');
...
assert(classifyImport.includes("update_staged_entry(staged_import, entry['staged'], {'pendingDestination': destination})") && classifyImport.includes("'committedDestination': destination") && classifyImport.includes('committed_outputs'), 'work imports must journal per-file commits and resume without duplicate destinations');
...
assert(toolViews.includes('已选择 {paths.length} 个视频') && toolViews.includes('高级 · 批量粘贴路径') && toolViews.includes('removeVideoPath(item.path)') && toolViews.includes("label: '编码中'") && toolViews.includes("label: '失败'"), 'video transcoding inputs must use a grouped queue with removable items and per-file task states while keeping raw path entry advanced');
assert(projectWorkspace.includes("const VideoTrimTimeline") && projectWorkspace.includes("onChange={changeVideoTrimEdge}") && projectWorkspace.includes('另存为新视频') && projectWorkspace.includes('替换原视频') && !projectWorkspace.includes('fixed inset-0 z-[540]') && workspaceIpc.includes("request.saveMode === 'replace'") && workspaceIpc.includes('await fs.promises.rename(safeGenerated, sourcePath)') && workspaceIpc.includes('.photoflow-trim'), 'video trimming must use an in-preview dual-handle timeline and offer save-as-new or atomic source replacement');
...
assert(lines(app) < 1100, 'App.tsx exceeded the architecture size budget');
...
assert((projectWorkspace.match(/projectWorkflows && <button className="project-menu-item"[\s\S]{0,250}?setPanel\('negative-import'\)[\s\S]{0,100}?<\/button>/g) || []).length === 2, 'negative import must only appear in project file-browser menus, never in the inspiration library');
...
assert(workspaceIpc.includes('thumbnailService.syncChangedPaths(projectRoot, outputPaths') && workspaceIpc.includes("root: workspaceRoot, fileName: path.relative(workspaceRoot, projectRoot), eventType: 'rename'"), 'screenshot main-image output must queue fresh thumbnails and publish a workspace-root-relative file change');
...
assert(/operation === 'paste'[\s\S]*?await readSystemFileClipboard\(\)[\s\S]*?createProjectFileTask\([\s\S]*?resources: \[destinationDir, \.\.\.uniqueSources\][\s\S]*?await task\.start\(\)/.test(filesIpc), 'paste must lock both its resolved destination and clipboard sources before filesystem mutation');
...
assert(projectWorkspace.includes('设为版本进度…') && projectWorkspace.includes("fileMenu.entry.kind === 'folder'"), 'any ordinary project folder must be available as a V2 relationship source');
...
assert(toolViews.includes('多个时段可以选择同一个项目') && !toolViews.includes('selectedProjects.size !== dateGroups.length'), 'adaptive shooting segments must remain independently routable to the same project');
