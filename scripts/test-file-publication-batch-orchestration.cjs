...
const run = async () => {
  ...
  try {
    ...
    const deletedCleanup = await makePlan(root, 'deleted-cleanup', 3); let deletedCleanupCalls = 0; let deletedCleanupPaths = [];
    const deletedCleanupNative = { inspectPathsBatch: async paths => inspect(paths), moveNoReplaceBatch: async () => { throw Object.assign(new Error('publication stopped before commit'), { code: 'BATCH_STOPPED', failedIndex: 0, completed: [] }); }, deletePathsBatch: async requests => { deletedCleanupCalls += 1; deletedCleanupPaths = requests.map(request => request.path); return requests.map((request, index) => { fs.unlinkSync(request.path); return { index, success: false, deleted: true, cleanupWarning: true, phase: 'post-unlink-cleanup', outcomeUnknown: true }; }); } };
    await assert.rejects(copyPlannedFiles(deletedCleanup.plan, { nativePublicationService: deletedCleanupNative }), error => error.code === 'BATCH_STOPPED' && error.cleanupWarning === true && error.cleanupPhase === 'post-unlink-cleanup' && error.outcomeUnknown === true && (error.recoveryPaths || []).length === 0 && error.deletedCleanupCount === 3);
    assert.strictEqual(deletedCleanupCalls, 1, 'post-unlink batch cleanup warning is consumed once without retry'); assert.strictEqual(deletedCleanupPaths.length, 3); assert(deletedCleanupPaths.every(candidate => !fs.existsSync(candidate)), 'batch cleanup recoveryPaths must never reference post-unlink staging objects');
