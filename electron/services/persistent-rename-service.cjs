...
const createPersistentRenameService = ({ launch, timeoutMs = 120000, idleMs = 60000, terminate = child => terminateAndWait(child, Date.now() + 5000) }) => {
  let session = null;
  let sequence = 0;
  ...
  const start = async () => {
    ...
    child.stdout.on('data', chunk => {
      ...
      if (current.buffer.length > 512 * 1024) { void fail(new Error('文件改名服务响应过大')); return; }
      let newline;
      while ((newline = current.buffer.indexOf('\n')) >= 0) {
        ...
        if (!current.handshake) {
          if (payload.protocol !== 'photoflow-rename-v1' || payload.ready !== true) { void fail(new Error('文件改名服务协议不兼容')); return; }
  ...
  const move = (source, target) => {
    const result = tail.then(async () => {
      ...
      return new Promise((resolve, reject) => {
        const timer = setTimeout(() => void current.fail(Object.assign(new Error('文件改名响应超时，操作结果未知'), { code: 'FILE_PUBLICATION_TIMEOUT' })), timeoutMs);
        current.pending = { id, resolve, reject, timer };
        try { current.child.stdin.write(JSON.stringify({ id, source, target }) + '\n'); }
  ...
  return { move, warm: start, stop: async () => { stopped = true; if (session) await session.fail(new Error('文件改名服务已停止')); } };
};
