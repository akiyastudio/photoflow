...
const createFilePublicationService = ({ app, projectRoot, processSupervisor = null, platform = process.platform, invokeOverride = null, spawnImpl = null, batchTimeoutMs = publicationBatchTimeoutMs, portableFaultInjector = async () => undefined }) => {
  ...
  const copyFilesNativeBatch = async (requests, options = {}) => {
    ...
    try {
      ...
      const payload = await invoke(operation, { manifest, 'manifest-size': Buffer.byteLength(contents), 'manifest-sha256': crypto.createHash('sha256').update(contents).digest('hex'), ...(options.durable === true ? { durable: 'true' } : {}) }, 30 * 60 * 1000);
      const results = Array.isArray(payload?.results) ? payload.results : [];
      ...
      for (const [index, item] of [...indexed].sort((left, right) => left[0] - right[0])) {
        ...
        completed.push({ index, identity: item.identity, sha256: item.sha256, sourceDeleted: deleteSource, published: true });
      }
  ...
  const copyFilesBatch = (requests, options = {}) => copyFilesNativeBatch(requests, { durable: options.durable === true });
  const copyCutFilesBatch = requests => copyFilesNativeBatch(requests, { deleteSource: true, durable: true });
