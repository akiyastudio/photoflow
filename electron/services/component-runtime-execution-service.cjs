...
const createComponentRuntimeExecutionService = ({ broker, ensureWorkspace, getProjectPath, getWorkspaceDataRoot, getBoundProject, path, fs, crypto, inputTokens, pluginService, backgroundTasks, versionService, mainWindow, resolveComponentContentBinding = null }) => {
  ...
  const bound = async (context, descriptor) => {
    ...
    return { workspaceRoot, project, projectRoot, scopeRoot, canonicalScope, componentRoot: path.join(getWorkspaceDataRoot(workspaceRoot), 'components', descriptor.componentId), key: `${descriptor.componentId}\0${workspaceRoot}\0${project.id}\0${suppliedScope}` };
  };
  const resolveInputs = async (payload, context, descriptor, scope, reservedTokenFiles = null) => {
    ...
    if (relativePaths.length + tokenValues.length > 120) throw hostError(CODES.INVALID_REQUEST, 'Too many component runtime inputs');
  ...
  const invoke = async (payload, context, descriptor) => {
    fields(payload,['action','runtimeCapability','arguments','relativePaths','inputTokens','input','operationKey','idempotencyKey','eventName','timeoutMs','task','control','projectArtifacts'],['action']);const action = String(payload?.action || ''); const scope = await bound(context, descriptor);
    ...
    if (action === 'inputs.preview') {
      fields(payload,['action','relativePaths','inputTokens','input'],['action','input']);fields(payload.input,['extensions','prefixArgumentCount','directoryArgument']);
    ...
    const invocationArgs=[...args.slice(0,prefixCount),...inputs.map(item=>item.filePath),...args.slice(prefixCount),...(directoryArgument?inputs.filter(item=>item.directory).flatMap(item=>[directoryArgument,item.filePath]):[]),...controlArgs];
    ...
    const execute = async task => {
      ...
      try {
        const final = await pluginService.runJsonForComponentCapability(
          ...
          Math.max(1000, Math.min(4 * 60 * 60 * 1000, Number(payload.timeoutMs) || 20 * 60 * 1000)),
          message => {
        ...
        if (payload.projectArtifacts) {
          ...
          if (final.projectArtifacts.linked.length && mainWindow && !mainWindow.isDestroyed()) {
            mainWindow.webContents.send('workspace-files-changed', { root: scope.workspaceRoot, fileName: path.relative(scope.workspaceRoot, scope.projectRoot).replace(/\\/g, '/'), eventType: 'rename' });
          }
        }
  ...
  return { invoke, clearComponent: componentId => {
