const registerComponentHostIpc = ({ ipcMain, manager, mainWindow }) => {
  ...
  ipcMain.handle('component-host-activate', (event, value) => { mainRenderer(event);if(plain(value)){exact(value,['instanceId','deactivateIfActive'],['instanceId','deactivateIfActive']);if(value.deactivateIfActive!==true)throw new TypeError('Invalid conditional component deactivation');return{success:manager.deactivateIfActive(instanceId(value.instanceId))};} return { success: manager.activate(instanceId(value)) }; });
