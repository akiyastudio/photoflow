...
const registerVersionIpc = context => {
  ...
  const mergeTeamPatchPhoto = async (workspaceRoot, status, projectName, request = {}) => {
    try {
      const bundle = await versionService.getPhoto(workspaceRoot, request.photoId);
      const projectId = String(request.componentProjectId || bundle.photo?.projectId || '');
      if (!projectId) throw new Error('团片项目身份不存在');
      return await componentServiceManager.invoke('team-retouch', 'team.patch.merge.v1', request, {
        componentId: 'team-retouch', componentVersion: 'host-internal', workspacePath: workspaceRoot,
        projectId, projectName, projectStatus: status,
      });
    } catch (error) {
  ...
  ipcMain.handle('workspace-team-patch-return-batch', async (event, workspacePath, status, projectName, request = {}) => {
    ...
    try {
      ...
      for (const [index, item] of touchedGroups.entries()) {
        ...
        const result = await mergeTeamPatchPhoto(workspaceRoot, status, projectName, {
          componentProjectId: request.componentProjectId,
