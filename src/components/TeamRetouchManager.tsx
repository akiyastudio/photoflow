...
type PatchHighlight = { x: number; y: number; width: number; height: number; personIndex: number };

const PatchImage = ({ path, cacheConfig, label, frame, highlights = [] }: { path?: string; cacheConfig: AppConfig['mediaCache']; label: string; frame?: { width: number; height: number }; highlights?: PatchHighlight[] }) => {
  ...
  return <div className="relative flex min-h-36 items-center justify-center overflow-hidden rounded-lg bg-slate-950">
    {url ? frame && highlights.length ? <svg role="img" aria-label={`${label}，黄色框为目标人物`} className="h-44 w-full" viewBox={`0 0 ${frame.width} ${frame.height}`} preserveAspectRatio="xMidYMid meet"><image href={url} width={frame.width} height={frame.height}/>{highlights.map(highlight => { const stroke = Math.max(3, frame.width / 450); const fontSize = Math.max(20, frame.width / 32); const labelWidth = fontSize * 2.8; const labelHeight = fontSize * 1.45; const labelY = Math.max(0, highlight.y - labelHeight); return <g key={highlight.personIndex}><rect x={highlight.x} y={highlight.y} width={highlight.width} height={highlight.height} rx={stroke * 2} fill="rgba(250,204,21,.08)" stroke="#facc15" strokeWidth={stroke}/><rect x={highlight.x} y={labelY} width={labelWidth} height={labelHeight} rx={stroke * 1.5} fill="rgba(0,0,0,.78)"/><text x={highlight.x + fontSize * .35} y={labelY + fontSize} fill="#fde047" fontSize={fontSize} fontWeight="700">目标 {highlight.personIndex}</text></g>; })}</svg> : <img src={url} alt={label} className="h-44 w-full object-contain"/> : <UserRound size={25} className="text-slate-500"/>}
...
const patchHighlights = (task: TeamPatchTask): PatchHighlight[] => {
  ...
  return members.map(member => {
    ...
    return {
      ...
      x,
      y,
      width: Math.max(1, Math.min(member.bbox.width, task.crop.width - x)),
      height: Math.max(1, Math.min(member.bbox.height, task.crop.height - y)),
    };
  });
};
