...
import { AlertTriangle, CheckCircle2, ExternalLink, Loader2, RefreshCw, ScanFace, Sparkles, Upload, UserRound, UsersRound, Wand2, X } from 'lucide-react';
...
const BatchTeamRetouchManager = ({ entries, workspacePath, project, onClose, onNotice, onOpen }: Props & { onOpen: (entry: ProjectFileEntry) => void }) => {
  ...
  return <div className="fixed inset-x-0 bottom-0 top-10 z-[310] flex flex-col bg-slate-50">
    <header className="flex min-h-16 items-center gap-3 border-b border-slate-200 bg-white px-5 py-3"><span className="rounded-xl bg-violet-50 p-2 text-violet-600"><UsersRound size={20}/></span><div><h2 className="font-bold text-slate-900">批量多人修脸 · {entries.length} 张图片</h2><p className="mt-0.5 text-xs text-slate-500">整批共用 PairDETR 与 SAM 2.1 模型，处理结束后自动关闭服务并释放显存。</p></div><div className="ml-auto flex gap-2"><button disabled={running} onClick={() => void runBatch()} className="dialog-primary inline-flex items-center gap-2">{running ? <Loader2 size={15} className="animate-spin"/> : <ScanFace size={15}/>} {results.length ? '重新批量识别' : '开始批量识别'}</button><button disabled={running} onClick={onClose} className="rounded-md p-2 text-slate-500 hover:bg-slate-100 disabled:opacity-40"><X size={20}/></button></div></header>
