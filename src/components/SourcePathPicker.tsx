...
export const SourcePathPicker = ({
  ...
  return <section className="space-y-3">
    ...
    >
      {!normalizedPaths.length ? <button type="button" disabled={disabled || loading || !onChooseFiles} onClick={() => void onChooseFiles?.()} className="flex min-h-28 w-full flex-col items-center justify-center gap-2 rounded-lg px-4 text-sm text-slate-500 hover:bg-white disabled:opacity-50"><FolderInput size={24} className="text-slate-400"/><span className="font-semibold">{loading ? '正在读取…' : emptyTitle}</span><span className="text-xs text-slate-400">{dragActive ? '松开即可追加' : emptyDescription}</span></button> : <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
