...
const SubjectThumb = memo(({ subject, cacheConfig, interactive = true, sourcePath }: { subject: Subject; cacheConfig: AppConfig['mediaCache']; interactive?: boolean; sourcePath?: string }) => {
  ...
  useEffect(() => {
    ...
    const observer = node ? new IntersectionObserver(([entry]) => {
      ...
      if (!request) {
        request = window.electronAPI.getMediaThumbnail(imagePath, 'image', cacheConfig, IDENTITY_THUMBNAIL_SIZE, 1, 0);
...
export const PersonIdentityManager = ({ workspacePath, project, cacheConfig, activeStep, onStepChange, onClose, onNotice, onProjectChanged, onBusyChange }: Props) => {
  ...
  const ignoreWorkflowReturn = async (match: TeamPatchReturnMatch) => {
    ...
    const result = await window.electronAPI.ignoreTeamWorkflowReturnReview(workspacePath, project.name, reviewSessionId, match.returnId);
    setBusy('');
    if (!result.success) {
