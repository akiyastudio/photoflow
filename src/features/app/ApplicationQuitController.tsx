  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (confirmed) { setState(current => ({ ...current, phase: 'saving' })); await flushApplicationBeforeQuit(); }
        await window.electronAPI.respondToApplicationQuit(requestId, confirmed);
      } catch (error) {
        await window.electronAPI.respondToApplicationQuit(requestId, false, `保存设置失败：${error instanceof Error ? error.message : String(error)}`);
      }
    })();
  }, [dialog, state]);
  const visible = showSaving || state.phase === 'failed';
  ...
  return <div className="fixed inset-0 z-[3100] flex items-center justify-center bg-slate-950/40 p-4">
    <section ref={surface} tabIndex={-1} onKeyDown={event => { if (event.key === 'Tab') { event.preventDefault(); (surface.current?.querySelector<HTMLButtonElement>('button') || surface.current)?.focus(); } }} role="dialog" aria-modal="true" aria-labelledby="application-quit-title" className="w-full max-w-md rounded-xl border border-slate-200 bg-white p-5 shadow-2xl">
      <div className="flex items-center gap-3">
        {state.phase === 'failed' ? <AlertTriangle className="text-amber-500" size={20}/> : <Loader2 className="animate-spin text-blue-500" size={20}/>}
        <h3 id="application-quit-title" className="font-bold text-slate-800">{state.phase === 'failed' ? '退出暂未完成' : '正在保存并结束任务'}</h3>
      </div>
      <p className="mt-3 text-sm leading-6 text-slate-500">{state.phase === 'failed' ? state.message : '请稍候，完成必要的保存后会自动关闭。'}</p>
      {state.phase === 'failed' && <div className="mt-5 flex justify-end"><button type="button" className="dialog-primary" onClick={() => window.electronAPI.closeWindow()}>重试退出</button></div>}
    </section>
  </div>;
};
