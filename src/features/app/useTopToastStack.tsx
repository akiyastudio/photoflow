...
import { hostNoticeTone, topToastTonePresentation } from './top-toast-tone-model';
...
export const useTopToastStack = () => {
  ...
  const reportToastReservation = useCallback((forcedBottom?: number) => {
    ...
    const bottom = forcedBottom ?? hostToastReservationBottom(stack, 12, window.innerHeight);
  ...
  const showNotice = useCallback((message: string, duration = 3500, explicitTone?: TopToastNotice['tone']) => {
    const cleanMessage = message.trim() || '发生未知错误';
    const tone = explicitTone || hostNoticeTone(cleanMessage);
    ...
    enqueueNotice({ id, message: cleanMessage, persistent: isFailure, count: 1, tone });
