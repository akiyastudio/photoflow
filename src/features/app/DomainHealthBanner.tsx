...
const DOMAIN_LABELS: Record<string, string> = {
  ...
  'tracking-scan': '版本跟踪', 'team-retouch': '团片协作',
};

export const DomainHealthBanner = ({ onNotice }: { onNotice: (message: string, duration?: number) => void }) => {
  ...
  return <div role="status" className={`relative z-40 flex shrink-0 items-center gap-3 border-b px-4 py-2 text-xs ${hasFailure ? 'border-red-200 bg-red-50 text-red-800' : 'border-amber-200 bg-amber-50 text-amber-800'}`}>
    ...
    <span className="min-w-0 flex-1 truncate"><strong>部分功能已隔离：</strong>{domains.map(domain => `${DOMAIN_LABELS[domain.domainId] || domain.domainId}${domain.state === 'recovering' ? '正在恢复' : domain.state === 'degraded' ? '已降级' : '暂不可用'}`).join('、') || '跨域任务等待处理'}。其他功能仍可继续使用。</span>
