...
export const TrackingConfirmationPanel = ({ active, sessionId, workspacePath, progressFolders, cacheConfig, onClose, onCommitted, onReleased, onNotice }: TrackingConfirmationPanelProps) => {
  ...
  const decide = async (status: 'accepted' | 'rejected', referenceName?: string) => {
    if (!selectedItem || actionInFlightRef.current) return;
  ...
  const commit = () => {
    if (actionInFlightRef.current || loadError || busyItemId || (view.session?.unresolvedCount ?? 1) > 0 || !canCommitTrackingSession(view.items)) return;
    actionInFlightRef.current = true;
    const generation = actionGenerationRef.current;
    setCommitting(true);
  ...
  const commitUnavailable = loading || Boolean(loadError) || Boolean(busyItemId) || unresolved > 0 || !canCommitTrackingSession(view.items) || Boolean(view.session?.status === 'failed' && !view.items.length);
  const revealPending = () => {
  ...
  return <div role="dialog" aria-modal="true" aria-label="确认版本匹配" className="fixed inset-0 z-[350] flex items-center justify-center bg-slate-950/55 p-4">
    <div className="tracking-confirmation-dialog">
      ...
      <footer className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 bg-slate-50 px-5 py-4"><button type="button" disabled={committing || Boolean(busyItemId)} onClick={() => void release()} className="text-xs font-medium text-red-600 hover:text-red-700 disabled:opacity-40">放弃本次确认</button><div className="flex gap-2"><button type="button" disabled={committing || Boolean(busyItemId)} onClick={onClose} className="dialog-secondary">稍后继续</button>{commitUnavailable ? <button type="button" disabled={committing || loading || Boolean(loadError) || unresolved === 0} onClick={revealPending} className="dialog-primary">{loadError ? '无法提交' : '有待确认图片'}</button> : <button type="button" disabled={committing} onClick={() => void commit()} className="dialog-primary inline-flex items-center gap-2">{committing && <Loader2 size={15} className="animate-spin"/>}提交结果</button>}</div></footer>
    </div>
  </div>;
};
