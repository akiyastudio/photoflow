  ...
  return <div role="dialog" aria-modal="true" aria-label={definition.title} className="fixed inset-0 z-[340] flex items-center justify-center bg-slate-950/55 p-4">
    <section className="w-full max-w-[660px] overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl">
      ...
      <div className="space-y-4 px-5 py-4">
        ...
        {draft.versionKey && <div className="rounded-lg border border-blue-200 bg-blue-50 px-3 py-2 text-sm text-blue-800"><b>将创建 V{draft.versionKey}</b><span className="ml-2 text-xs text-blue-600">V1_1 版本分支与 V2 一样支持跟踪、比较和提交。</span></div>}
        <div><span className="mb-2 block text-xs font-semibold text-slate-600">节点用途</span><div className="grid grid-cols-2 gap-3"><button type="button" disabled={busy} aria-pressed={draft.relationKind === 'main'} onClick={() => { const next = normalizeTrackingPolicy('main', draft); update({ relationKind: 'main', ...next }); }} className={`rounded-lg border p-3 text-left ${draft.relationKind === 'main' ? 'border-blue-500 bg-blue-50' : 'border-slate-200'}`}><b className="block text-sm">版本进度</b><small className="text-slate-500">包含 V2 主线和 V1_1 版本分支，都可正常跟踪</small></button><button type="button" disabled={busy} aria-pressed={draft.relationKind === 'auxiliary'} onClick={() => update({ relationKind: 'auxiliary', ...normalizeTrackingPolicy('auxiliary', draft) })} className={`rounded-lg border p-3 text-left ${draft.relationKind === 'auxiliary' ? 'border-violet-500 bg-violet-50' : 'border-slate-200'}`}><b className="block text-sm">选片辅助节点</b><small className="text-slate-500">类似 JPG、mov_预览、团片协作，不是版本分支</small></button></div></div>
        <label className="block"><span className="mb-1 block text-xs font-semibold text-slate-600">父节点</span><select value={draft.parentProgressId} disabled={busy} onChange={event => update({ parentProgressId: event.target.value })} className="w-full rounded-md border border-slate-300 px-3 py-2 text-sm"><option value="">无父节点</option>{parents.map(folder => <option key={folder.id} value={folder.id}>{folder.displayName} · {folder.nodeRole === 'original' ? '原始素材' : folder.relationKind === 'auxiliary' ? '选片辅助节点' : folder.versionKey.includes('_') ? '版本分支' : '版本进度'}</option>)}</select></label>
