...
import { ConverterView, ImportCard, MatchView, ResearchView, ScreenshotMainImageView, VideoSplitView, VideoTranscodeView, type ImportCompletion, type VideoTranscodeFolderOutput } from '../tools/ToolViews';
...
import { addPendingFileOperation, applyPendingFileOperations, operationRefreshDirectories, pendingOperationForEntry, pendingPathConflicts, predictUniqueDirectoryName, removePendingFileOperation, type PendingFileOperation, type PendingProjectFileEntry } from './file-operation-state-model';
...
import { mediaContributionScope, projectContributionScope } from '../components/component-contribution-scope-model';
...
const FileBrowserWorkspace = ({ pageId, active, activeView, project, workspacePath, inspirationTargetWorkspacePath, inspirationLibraryRootPath, installedComponentIds: _installedComponentIds, componentHostActions = [], componentContributions = [], onOpenComponentPage = () => undefined, videoPlaybackSettings, projectToolbar = { order: [...PROJECT_TOOLBAR_ACTION_IDS], hidden: [], onlyShowAvailable: false }, customProjectCategories = [], projectCategoryOrder = [], progressNamePresets = [], initialPanel, initialRelativePath = '', importConfig, importDefaults, brollConfig, videoTools, matchConfig, researchConfig, mediaCacheConfig, defaultFolderSort, itemOpenMode, folderAlphabetFilterEnabled = true, versionTreeEnabled = true, favoriteDisplayMode = 'binary', browserContext, navigationRequest, onDirectoryChange, onOpenInspirationPath, onOpenDirectoryPage, onOpenToolTab = () => undefined, onCloseToolTab = () => undefined, onImportConfigChange, onMatchConfigChange, onResearchConfigChange, onNotice, onProjectMoved = () => undefined, onDeleted = () => undefined }: FileBrowserWorkspaceProps) => {
  ...
  const [videoTranscodeBusy, setVideoTranscodeBusy] = useState(false);
  ...
  const commitBatchRename = async () => {
    ...
    const pendingOperation = startPendingFileOperation({
      ...
      affectedDirectories: [currentRelativePath], tombstonePaths: sourcePaths, optimisticEntries,
    });
    if (!pendingOperation) return;
  ...
  const runFileOperation = async (operation: 'trash' | 'copy' | 'cut' | 'paste' | 'rename', nextName?: string, targetPaths = selectedPaths, destinationRelativePath = operationDirectoryPath) => {
    ...
    const pendingOperation = operation === 'rename' ? null : startPendingFileOperation({
      ...
      lockedPaths: operation === 'paste'
        ? [`__directory__/${normalizedDestination || '__root__'}`, ...(normalizedDestination ? [normalizedDestination] : [])]
        : targetPaths,
  ...
  useEffect(() => {
    ...
    void (async () => {
      for (const target of targets) {
        const result = await projectWorkspaceClient.getProjectEntryDetails(workspacePath, project.status, project.name, target.relativePath);
        if (!active) return;
        if (result.success && result.details) {
          setSelectionEntryDetails(current => ({ ...current, [target.path]: result.details! }));
        }
      }
      if (active) setSelectionEntryDetailsLoading(false);
    })();
    return () => { active = false; };
  ...
  const openVideoTranscode = async (targetRelativePaths = selectedPaths) => {
    ...
    while (sequence === videoTranscodeInspectionSequenceRef.current) {
      ...
      if (result.indexed) {
        const sources = resolveInspectedToolSources(result.sources, result.folderPaths, result.videoPaths);
        setVideoTranscodeTargets(sources.map(source => source.path));
        setVideoTranscodeSourceFolders(sources.filter(source => source.kind === 'folder').map(source => source.path));
        setVideoTranscodeCollecting(false);
  ...
  const registerVideoTranscodeOutputs = useCallback(async (folderOutputs: VideoTranscodeFolderOutput[]) => {
    const normalizedProjectRoot = project.path.replace(/\\/g, '/').replace(/\/+$/g, '');
    const projectRootKey = normalizedProjectRoot.toLocaleLowerCase('zh-CN');
    const latestFolders = await loadProgressFolders();
    const folders = latestFolders.length ? latestFolders : progressFoldersRef.current;
    let registeredCount = 0;
    const skipped: string[] = [];
    for (const mapping of folderOutputs) {
      const sourceKey = mapping.sourceFolder.replace(/\\/g, '/').replace(/\/+$/g, '').toLocaleLowerCase('zh-CN');
      const source = folders.find(folder => folder.folderPath.replace(/\\/g, '/').replace(/\/+$/g, '').toLocaleLowerCase('zh-CN') === sourceKey);
      const sourceIsValid = source?.mediaKind === 'video' && !source.folderMissing
        && (source.nodeRole === 'original' && !source.artifactKind
          || source.nodeRole === 'progress' && Boolean(source.parentProgressId) && source.relationKind === 'main');
      const normalizedOutput = mapping.outputFolder.replace(/\\/g, '/').replace(/\/+$/g, '');
      const outputKey = normalizedOutput.toLocaleLowerCase('zh-CN');
      if (!sourceIsValid || !outputKey.startsWith(`${projectRootKey}/`)) {
        skipped.push(normalizedOutput.split('/').pop() || normalizedOutput);
        continue;
      }
      const relativePath = normalizedOutput.slice(normalizedProjectRoot.length + 1);
      const adopted = await projectWorkspaceClient.adoptVersionTreeFolder(workspacePath, project.status, {
        projectName: project.name,
        relativePath,
        mode: 'transcode',
        mediaKind: 'video',
      ...
      if (!adopted.success || !adopted.progressFolder) {
        skipped.push(`${relativePath}：${adopted.error || '登记失败'}`);
        continue;
      }
      registeredCount += 1;
    }
    ...
    if (skipped.length) onNotice(`转码文件已生成，但 ${skipped.length} 个输出未进入版本树：${skipped[0]}`, 7000);
    else if (registeredCount) onNotice(`已建立 ${registeredCount} 条转码产物连线。`);
  }, [loadProgressFolders, onNotice, project.name, project.path, project.status, refresh, workspacePath]);
  ...
  const fileMenuHasToolActions = Boolean(fileMenu && (fileMenuHasVideoTarget || fileMenuHasConvertibleImageTarget || fileMenuScreenshotMainImageEntries.length || fileMenuOfficeEntries.length));
  ...
  return (
    <div ref={projectWorkspaceRef} className="flex h-full w-full min-w-0 flex-col animate-in fade-in duration-300">
      ...
      {fileMenu && createPortal(<ViewportContextMenu x={fileMenu.x} y={fileMenu.y} widthClass="w-52" allowSubmenus>
        ...
        {(fileMenuHasConvertibleImageTarget || fileMenuScreenshotMainImageEntries.length > 0) && <div className="group/submenu relative"><button type="button" className="project-menu-item w-full"><ImageIcon size={14}/>图片工具<span className="ml-auto">›</span></button><div className="invisible absolute left-full top-0 z-[302] ml-1 w-52 rounded-lg border border-slate-200 bg-white p-1 opacity-0 shadow-xl transition group-hover/submenu:visible group-hover/submenu:opacity-100">{fileMenuHasConvertibleImageTarget && <button className="project-menu-item" onClick={() => { const targets = fileMenuEntries.map(entry => entry.relativePath); setFileMenu(null); void openImageConverter(targets); }}><ImageIcon size={14}/>图片转 JPG</button>}<button disabled={!fileMenuScreenshotMainImageEntries.length} title={fileMenuScreenshotMainImageEntries.length ? fileMenuScreenshotMainImageEntries.length > 1 ? `批量提取 ${fileMenuScreenshotMainImageEntries.length} 张截图中的主图` : '提取截图中的主图' : '需选择截图图片使用'} className="project-menu-item" onClick={() => { const entries = fileMenuScreenshotMainImageEntries; setFileMenu(null); openScreenshotMainImage(entries); }}><Crop size={14}/>提取截图主图{fileMenuScreenshotMainImageEntries.length > 1 ? `（${fileMenuScreenshotMainImageEntries.length} 张）` : ''}</button></div></div>}
      ...
      {surfaceMenu && createPortal(<ViewportContextMenu x={surfaceMenu.x} y={surfaceMenu.y} widthClass="w-56" allowSubmenus>
        ...
        {componentContributions.filter(item => item.type === 'project.contextAction').map(item => <button key={`${item.componentId}:${item.contributionId}`} className="project-menu-item" onClick={() => { const scope = projectContributionScope(surfaceMenu.targetRelativePath, pageId); setSurfaceMenu(null); window.dispatchEvent(new CustomEvent('photoflow:open-component-contribution', { detail: { contribution: item, scope } })); }}><Plus size={14}/>{item.label}</button>)}
        ...
        <div className="group/submenu relative"><button className="project-menu-item w-full"><FolderInput size={14}/>导入<span className="ml-auto">›</span></button><div className="invisible absolute left-full top-0 z-[302] ml-1 w-52 rounded-lg border border-slate-200 bg-white p-1 opacity-0 shadow-xl transition group-hover/submenu:visible group-hover/submenu:opacity-100">{projectWorkflows && <button className="project-menu-item" onClick={() => { setSurfaceMenu(null); setPanel('import'); }}><MemoryStick size={14}/>从 SD 卡导入</button>}<button className="project-menu-item" onClick={() => { const target = surfaceMenu.targetRelativePath; setSurfaceMenu(null); openManualImport(projectWorkflows ? 'original' : 'files', [], target); }}><FolderInput size={14}/>导入</button>{componentContributions.filter(item => item.type === 'project.importProvider').map(item => <button key={`${item.componentId}:${item.contributionId}`} className="project-menu-item" onClick={() => { const scopeRelativePath = surfaceMenu.targetRelativePath; setSurfaceMenu(null); window.dispatchEvent(new CustomEvent('photoflow:open-component-contribution', { detail: { contribution: item, scope: { scopeRelativePath, selectedRelativePaths: [], sourcePageId: pageId } } })); }}><FolderInput size={14}/>{item.label}</button>)}</div></div>
      ...
      <div className={`project-toolbar flex w-full flex-nowrap items-center border-b border-slate-200 px-6 py-1 ${selectedPaths.length ? 'project-toolbar--has-selection' : ''}`}>
        ...
        <div className="project-toolbar-component-actions contents">{projectWorkflows && <ComponentToolbarActions actions={componentHostActions} scope={{ scopeRelativePath: currentRelativePath, selectedRelativePaths: selectedEntries.filter(entry => entry.kind === 'image').map(entry => entry.relativePath), sourcePageId: pageId }} onOpen={onOpenComponentPage}/>}<ComponentContributionDock contributions={componentContributions.filter(item => item.type !== 'application.command')} project={project} workspacePath={workspacePath} scope={{ scopeRelativePath: currentRelativePath, selectedRelativePaths: selectedEntries.map(entry => entry.relativePath), sourcePageId: pageId }} active={active}/></div>
      ...
      {mountedPanels.has('video-transcode') && <ToolModal title={PROJECT_PANEL_TITLES['video-transcode']} ownerPageId={pageId} panelKind="video-transcode" open={panel === 'video-transcode'} busy={videoTranscodeBusy} useBackgroundTaskBusyFallback={false} onClose={() => { videoTranscodeInspectionSequenceRef.current += 1; setVideoTranscodeCollecting(false); setPanel(null); }}><VideoTranscodeView embedded initialTargetPaths={videoTranscodeTargets} initialSourceFolders={videoTranscodeSourceFolders} sourcesLoading={videoTranscodeCollecting} onBusyChange={setVideoTranscodeBusy} onFolderTranscodeComplete={registerVideoTranscodeOutputs}/></ToolModal>}
...
const FileMetadataPane = ({ entry, selectedEntries, selectionEntryDetails, selectionEntryDetailsLoading, entryDetails, metadataFields, metadataLoading, metadataError, technicalMetadata, formatFileSize, width, pinned, onTogglePinned, onOpen, onCopyPath, onClose }: {
  entry?: ProjectFileEntry;
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-3 py-2">
      {multiSelection ? <>
        <section className="grid grid-cols-2 gap-1.5 py-2">
          {([
            ['已选项目', String(selectionSummary.selectedCount), CheckSquare],
            ['合计大小', selectionSizeLabel, MemoryStick],
            ['文件', String(selectionSummary.selectedFileCount), FileText],
            ['文件夹', String(selectionSummary.selectedFolderCount), Folder],
        ...
        <section className="border-b border-slate-200">
          <div className="flex w-full items-center gap-2 py-2.5 text-left"><CheckSquare size={13} className="text-slate-400"/><span className="text-xs font-bold text-slate-700">所选内容</span><span className="ml-auto text-[10px] text-slate-400">{[selectionCreatedRange, selectionUpdatedRange].filter(Boolean).length + 6}</span></div>
          <dl className="pb-2">
            <MetadataRow label="选中" value={`${selectionSummary.selectedCount} 个项目`}/>
            <MetadataRow label="包含" value={selectionContainedLabel}/>
            <MetadataRow label="类型" value={selectionSummary.typeSummary || '—'}/>
            <MetadataRow label="文件格式" value={selectionSummary.formatSummary}/>
            <MetadataRow label="合计大小" value={selectionSizeLabel}/>
            <MetadataRow label="所在位置" value={selectionSummary.commonParentPath || '项目根目录'}/>
            {selectionCreatedRange && <MetadataRow label="创建时间" value={selectionCreatedRange}/>}
            {selectionUpdatedRange && <MetadataRow label="修改时间" value={selectionUpdatedRange}/>}
          </dl>
        </section>
