...
import { collapseRetryPredecessors, compareTaskCenterPublishedAt, formatBackgroundTaskStartedAt, isBackgroundTaskCenterVisible } from './task-toast-model';
        ...
        {visibleTaskItems.map(item => {
          ...
          return <div key={`background:${task.id}`} className="rounded-lg border border-slate-100 p-2.5">
            ...
            <div className="mt-2 flex justify-end gap-1">
              ...
              {task.state === 'failed' && !task.retryPending && <button type="button" onClick={() => void runAction(() => dismissBackgroundTask(task.id))} className="rounded px-2 py-1 text-[11px] text-slate-500 hover:bg-slate-100">清除</button>}
              {(task.state === 'completed' || task.state === 'cancelled' || task.state === 'interrupted') && <button type="button" onClick={() => void runAction(() => dismissBackgroundTask(task.id))} className="rounded px-2 py-1 text-[11px] text-slate-500 hover:bg-slate-100">清除</button>}
