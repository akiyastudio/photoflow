...
let mainWindow;
const getLogDir = () => { const dir = path.join(app.getPath('userData'), 'photoflow', 'logs'); fs.mkdirSync(dir, { recursive: true }); return dir; };
const writeLog = (level, message) => { try { const now = new Date(); fs.appendFileSync(path.join(getLogDir(), `photoflow-${now.toISOString().slice(0,10)}.log`), `[${now.toISOString()}] [${level}] ${message}\n`, 'utf8'); } catch (_) {} };
const cleanupLogs = () => { try { const deadline = Date.now() - 7 * 86400000; fs.readdirSync(getLogDir()).forEach(name => { const f = path.join(getLogDir(), name); if (name.endsWith('.log') && fs.statSync(f).mtimeMs < deadline) fs.unlinkSync(f); }); } catch (_) {} };
...
ipcMain.handle('set-theme', async (_event, theme) => { if (mainWindow) mainWindow.setTitleBarOverlay({ color: theme === 'dark' ? '#0f172a' : '#f8fafc', symbolColor: theme === 'dark' ? '#e2e8f0' : '#334155', height: 32 }); });
