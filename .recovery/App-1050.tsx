    ...
    const loadConfig = async () => {
      try {
        if (window.electronAPI?.loadConfig) {
          ...
          if (fileConfig) {
            let normalizedConfig = { ...fileConfig, theme: fileConfig.theme ?? 'system', workspacePath: fileConfig.workspacePath ?? 'D:/', smartImport: { ...fileConfig.smartImport, backupEnabled: false, generateVideoPreview: fileConfig.smartImport.generateVideoPreview ?? false } } as AppConfig;
  ...
  return (
    <div className="flex h-screen w-full bg-slate-50 text-slate-900 font-sans selection:bg-blue-500/30">
      ...
      <main className="flex-1 overflow-auto bg-slate-50 p-8 relative">
        {activeTab === 'home' && <div className="mx-auto max-w-6xl space-y-4"><DashboardView workspacePath={config.workspacePath} config={config.smartImport} onUpdateConfig={(newConfig: AppConfig['smartImport']) => handleConfigUpdate({...config, smartImport: newConfig})} /><HomePanel title="调研整理"><ResearchView embedded config={config.research} onUpdateConfig={(newConfig: AppConfig['research']) => handleConfigUpdate({ ...config, research: newConfig })}/></HomePanel><HomePanel title="PNG 转 JPG"><ConverterView embedded /></HomePanel></div>}
...
const DashboardView = ({
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <HomePanel title="从 SD 卡导入" initiallyOpen>
...
const HomePanel = ({ title, initiallyOpen = false, children }: { title: string; initiallyOpen?: boolean; children: React.ReactNode }) => {
  const [open, setOpen] = useState(initiallyOpen);
  return <section className="rounded-xl border border-slate-200 bg-white overflow-hidden"><button onClick={() => setOpen(value => !value)} className="flex w-full items-center justify-between px-5 py-4 text-left"><span className="text-base font-bold text-slate-800">{title}</span><span className="text-slate-400">{open ? <ChevronUp size={18}/> : <ChevronDown size={18}/>}</span></button>{open && <div className="border-t border-slate-100 p-5 animate-in slide-in-from-top-1 duration-200">{children}</div>}</section>;
};
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 animate-in fade-in duration-300">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><p className="text-sm font-medium text-blue-600">{project.status}</p><h2 className="mt-1 text-2xl font-bold text-slate-800">{project.name}</h2><p className="mt-1 text-sm text-slate-500">项目内容按名称正序排列</p></div>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-6">
        ...
        {folders.length ? <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">{folders.map(folder => <button key={folder.path} onClick={() => openFolder(folder.name)} className="flex items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 p-4 text-left transition hover:border-blue-300 hover:bg-blue-50"><FolderInput size={22} className="text-blue-600"/><span className="min-w-0 flex-1 truncate font-medium text-slate-700">{folder.name}</span><ExternalLink size={15} className="text-slate-400"/></button>)}</div> : <p className="py-8 text-center text-sm text-slate-400">当前项目还没有子文件夹。</p>}
      </section>
    </div>
  );
};
