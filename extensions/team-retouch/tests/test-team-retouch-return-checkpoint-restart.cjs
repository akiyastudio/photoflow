...
let tokenCounter = 0; const capabilities = {
  ...
  'project.input.tokens': payload => { const tokens = payload.action === 'materializeBatch' ? payload.tokens : [payload.token]; const items = tokens.map(token => { const source = token === 'base-token' ? basePath : token === 'completed-token' ? completedReturn : pendingReturn; const directory = path.join(dataPath,'inputs',String(++tokenCounter)); fs.mkdirSync(directory,{recursive:true}); const privatePath=path.join(directory,path.basename(source)); fs.copyFileSync(source,privatePath); return {inputId:String(tokenCounter),privatePath}; }); return payload.action === 'materializeBatch' ? {items} : items[0]; },
};
