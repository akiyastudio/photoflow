...
try {
  ...
  const workingPlan = { version: 1, kind: 'working-output', projectId: sourceId, preHostLocalEffects: 'none', ledgerPath: path.join(sourceComponentRoot, 'output-ownership', sourceHash, 'working-images.json'), outputRelativePath: '团片协作/old-working.png', domain: { version: 1, photoId: 'restored-photo', baseVersionId: 'restored-base', allTaskIds: ['restored-task'], task: { id: 'restored-task', personIndex: 1, personName: 'Restored', members: [{ personIndex: 1 }], generation: { version: 2 } }, photo: { displayName: 'Restored', relativePath: 'restored.jpg', relativePathState: 'ready', fileMissing: false } } };
