...
let status = { success: true, advancedAvailable: true, state: 'ready', advancedError: '' };
const rpcMethods = [];
globalThis.window = {
  photoFlowComponent: {
    contractVersion: 1,
...
assert.equal(componentStatusFromAdvancedPreflight({ success: true, message: 'checking' }).advancedState, 'unavailable', 'unknown state never guesses installation state');
assert.equal(componentStatusFromAdvancedPreflight({ success: true, advancedAvailable: false, state: 'repair-needed', advancedError: 'service failed' }).advancedAvailable, false);
assert.deepEqual(
...
status = { success: true, advancedAvailable: false, state: 'not-installed', advancedError: '增强人物检测尚未安装' };
