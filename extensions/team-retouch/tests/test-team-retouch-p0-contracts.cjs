...
(async () => {
  ...
  const simulator = createHostSimulator({
    ...
    capabilities: {
      'component.storage.v7': () => ({ apiVersion: 2, dataPath, databasePath, projectId: 'durable-project', ownership: 'component-private' }),
      'tasks.v7': () => { taskCapabilityCalls += 1; return { apiVersion: 2, task: null, cancelled: false }; },
    },
  });
  try {
