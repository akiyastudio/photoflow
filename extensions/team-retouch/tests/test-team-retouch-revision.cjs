...
const projectPath = projectId => path.join(dataPath, 'projects', crypto.createHash('sha256').update(projectId).digest('hex'), 'media', 'photo-a', 'version-a', 'working-a.png');
const workingPath = projectPath('project-a'); const workingPathB = projectPath('project-b');
for (const item of [workingPath, workingPathB]) { fs.mkdirSync(path.dirname(item), { recursive: true }); fs.writeFileSync(item, 'working'); }
...
db.prepare(`INSERT INTO team_patch_tasks(project_id,id,photo_id,base_version_id,person_index,person_name,bbox_json,crop_json,patch_path,members_json,generation_json,created_at,updated_at) VALUES(?,?,?,?,?,?,'{}','{}',?,'[{"personIndex":1}]','{"version":2}',1,1)`).run('project-a', 'task-a', 'photo-a', 'version-a', 1, '项目 A 人物', workingPath);
db.prepare(`INSERT INTO team_patch_tasks(project_id,id,photo_id,base_version_id,person_index,person_name,bbox_json,crop_json,patch_path,members_json,generation_json,created_at,updated_at) VALUES(?,?,?,?,?,?,'{}','{}',?,'[{"personIndex":1}]','{"version":2}',1,1)`).run('project-b', 'task-a', 'photo-a', 'version-a', 1, '项目 B 人物', workingPathB);
db.close();
...
const simulator = projectId => createHostSimulator({
  ...
  capabilities: { 'component.storage': () => ({ dataPath, databasePath, projectId, ownership: 'component-private' }) },
});
...
(async () => {
  ...
  try {
    ...
    await projectA.request('team.identity.suggest.v1', { acceptOnly: true, operationId: 'shared-operation', expectedRevision: '2' });
    await projectB.request('team.patch.detect.v1', { acceptOnly: true, operationId: 'shared-operation', photoId: 'photo-a', baseVersionId: 'version-a', expectedRevision: '1' });
    assert.equal((await projectA.request('team.operation.get.v1', { operationId: 'shared-operation' })).operation.kind, 'identity-suggest');
    assert.equal((await projectB.request('team.operation.get.v1', { operationId: 'shared-operation' })).operation.kind, 'detect', 'same operation id remains independently owned by each project');
    ...
    db.prepare('INSERT INTO team_project_revision_leases(project_id,request_id,expires_at) VALUES(?,?,?)').run('project-a', 'exclude-request', Date.now() + 60_000);
  ...
  } finally { await Promise.all([projectA.close(), projectB.close()]); try { fs.rmSync(sandbox, { recursive: true, force: true, maxRetries: 8, retryDelay: 75 }); } catch {} }
