...
assert(steps.includes('disabled={disabled || locked}') && steps.includes('data-selected={selected || undefined}') && steps.includes('data-tone={tone}') && steps.includes('stepIndex > progressIndex'), 'window selection, workflow progress, and locked future tabs must be independent states');
