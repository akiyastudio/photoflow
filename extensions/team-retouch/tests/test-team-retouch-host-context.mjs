...
import { createActivationRefreshGate, createHistoryContextLoadCoordinator, createLatestHistoryLoadGuard, historyLoadContextSignature, historyLoadPresentation, historyMigrationDelayMs } from '../renderer/src/legacy/legacy-history-load-model.ts';
import { legacyMigrationMadeProgress, legacyMigrationPausedMessage, legacyMigrationRemainingCount, legacyMigrationRunningMessage, nextLegacyMigrationNoProgressCount } from '../renderer/src/legacy/legacy-migration-progress-model.ts';
...
assert(entry.includes("rpc<Json>('team.project.migrate-step.v1'") && entry.includes('legacyMigrationRunningMessage'), 'legacy outputs migrate through incremental background requests after the first snapshot');
