...
const productionSetup = setup.slice(setup.indexOf('if ($TestHelpersOnly) { return }'));
assert.match(productionSetup, /Open-StagingDirectoryLock \$candidateRoot/);
assert.match(productionSetup, /Open-StagingDirectoryLock \$InstallRoot/);
assert.match(productionSetup, /Get-TrustedFileIdentity \$candidateVhd/);
assert.match(productionSetup, /Get-TrustedFileIdentity \$stableVhd/);
assert.doesNotMatch(productionSetup, /Get-Command tar\.exe/);
const candidateUnregister = productionSetup.slice(productionSetup.indexOf("Invoke-TestFault 'candidate-probe'"), productionSetup.indexOf('New-Item -ItemType Directory -Path $InstallRoot'));
assert(candidateUnregister.indexOf('Unregister-ReleasingLocks') < candidateUnregister.indexOf('$candidateRegistered = $false'));
assert.doesNotMatch(candidateUnregister.slice(candidateUnregister.indexOf('\n', candidateUnregister.indexOf('Unregister-ReleasingLocks')) + 1), /Get-TrustedFileIdentity|candidateVhd/);
const oldUnregister = productionSetup.slice(productionSetup.indexOf('$oldDigest ='), productionSetup.indexOf("Invoke-TestFault 'old-unregister'"));
assert(oldUnregister.indexOf('Close-VhdTransactionLocks') < oldUnregister.indexOf('Unregister-OwnedDistro'));
assert(oldUnregister.indexOf('Unregister-OwnedDistro') < oldUnregister.indexOf('$oldUnregistered = $true'));
assert(oldUnregister.indexOf('$oldUnregistered = $true') < oldUnregister.indexOf('Assert-DistroAbsent'));
