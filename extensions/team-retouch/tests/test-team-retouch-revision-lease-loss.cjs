...
const commonEnv = { PHOTOFLOW_TEST_REVISION_LEASES: '1', PHOTOFLOW_TEST_REVISION_LEASE_TTL_MS: '1400', PHOTOFLOW_TEST_REVISION_LEASE_RENEW_MS: '150' };
...
const deleteLease = async () => { for (let attempt = 0; attempt < 50; attempt += 1) { const db = new DatabaseSync(databasePath); try { db.exec('PRAGMA busy_timeout=100'); db.prepare('DELETE FROM team_project_revision_leases WHERE project_id=?').run(context.projectId); return; } catch (error) { if (!/locked/.test(String(error.message)) || attempt === 49) throw error; } finally { db.close(); } await sleep(10); } };

(async () => {
  try {
    const renewed = await first.request('team.test.revision-lease.v1', { delayMs: 2400, marker: 'renewed' });
    ...
    const blocked = first.request('team.test.revision-lease.v1', { delayMs: 1800, blockEventLoop: true, boundary: 'host', marker: 'blocked' });
    await assert.rejects(blocked, /租约已失效/, 'event-loop stalls beyond TTL fail closed');
    assert.equal(lifecycleCalls, 0);

    const oldDb = first.request('team.test.revision-lease.v1', { delayMs: 1800, marker: 'old-db' });
    await waitForLease(); await deleteLease();
    ...
    const oldFile = first.request('team.test.revision-lease.v1', { delayMs: 1800, boundary: 'file', marker: 'old-file' });
    await waitForLease(); await deleteLease();
    ...
    const oldJournal = first.request('team.test.revision-lease.v1', { delayMs: 1800, boundary: 'journal', marker: 'old-journal' });
    await waitForLease(); await deleteLease();
    ...
    const oldWorkflow = first.request('team.test.revision-lease.v1', { delayMs: 1800, boundary: 'workflow-stage', marker: 'old-workflow' });
    await waitForLease(); await deleteLease();
