...
const simulator=createHostSimulator({service:path.join(root,'service.cjs'),context:{componentId:'team-retouch',componentVersion:'test',surface:'project',projectId:'p',projectName:'P',projectStatus:'active'},capabilities:{
  'project.progress':payload=>{calls.push(payload);if(payload.action==='create'){outputCreated=true;return{apiVersion:7,progress:{id:'output',nodeRole:'progress',mediaKind:'image',missing:false},edges:[]};}return{apiVersion:7,progress:[{id:'original',nodeRole:'original',mediaKind:'image'},...(outputCreated?[{id:'output',nodeRole:'progress',mediaKind:'image',missing:false}]:[])],edges:[]};}
}});
