...
const App = () => {
  ...
  useEffect(() => {
    ...
    const stopActivate = window.photoFlowComponent.onActivate(() => { if (mounted) { setComponentActive(true); if (contextRef.current) void loadEntries(contextRef.current); } });
    const stopDeactivate = window.photoFlowComponent.onDeactivate(() => { if (mounted) setComponentActive(false); });
  ...
  return <LegacyDialogProvider><div className="legacy-root pf-canvas">{!entriesLoaded ? <TeamHistoryLoadSurface initialLoading={initialLoading} loadError={loadError} entriesLoaded={entriesLoaded} entryCount={entries.length} retry={retryHistory} openSettings={common.onOpenSettings}/> : step !== 'detect' ? <PersonIdentityManager {...common} onClose={common.onOpenSettings} activeStep={step}/> : <TeamRetouchManager {...common} historyRecordCount={historyRecordCount} historyOwnershipPendingCount={historyOwnershipPendingCount} entries={entries as any} onEntriesChange={value => setEntries(value)} />}{historyPathWarning && entriesLoaded && <div role="status" aria-live="polite" className="pf-banner team-banner fixed left-1/2 top-20 z-[819] flex max-w-3xl -translate-x-1/2 items-center gap-3 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-xs font-bold text-amber-700 shadow-xl"><AlertTriangle size={16}/><span>{historyPathWarning}</span><button type="button" aria-label="关闭路径提示" onClick={() => setHistoryPathWarning('')}>×</button></div>}{loadError && entriesLoaded && <div role="alert" className="pf-banner team-banner fixed left-1/2 top-20 z-[820] flex max-w-3xl -translate-x-1/2 items-center gap-3 rounded-lg border border-red-300 bg-red-50 px-4 py-3 text-xs font-bold text-red-700 shadow-xl"><AlertTriangle size={16}/><span>{loadError}</span><button type="button" className="rounded-md border border-red-300 px-2 py-1" onClick={retryHistory}>重试</button><button type="button" aria-label="关闭加载错误" onClick={() => setLoadError('')}>×</button></div>}{notice && <div role="status" aria-live="polite" className="pf-banner team-banner fixed bottom-5 left-1/2 z-[820] flex max-w-2xl -translate-x-1/2 items-center gap-3 rounded-lg bg-slate-900 px-4 py-3 text-sm font-bold text-white shadow-xl"><UsersRound size={16}/><span>{notice}</span><button className="ml-2 text-slate-300" onClick={() => setNotice('')}>×</button></div>}{settingsOpen && <TeamSettingsDialog value={settings} update={setSettings} close={() => setSettingsOpen(false)} notice={setNotice}/>}</div></LegacyDialogProvider>;
};
