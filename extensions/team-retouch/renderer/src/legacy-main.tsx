...
const TeamSettingsDialog = ({ value, update, close, notice }: { value: TeamSettings; update: (value: TeamSettings) => void; close: () => void; notice: (message: string) => void }) => {
  ...
  const save = (next: TeamSettings) => { update(next); void run('保存团片协作设置', async () => { assertSuccess(await rpc<Json>('component.settings.update.v1', next), '保存设置失败'); }); };
...
const App = () => {
  ...
  useEffect(() => {
    ...
    void rpc<Json>('component.settings.get.v1').then(saved => { if (mounted && saved.success !== false && saved.settings) setSettings(saved.settings as TeamSettings); }).catch(error => { if (mounted) setNotice(`读取团片设置失败：${error instanceof Error ? error.message : String(error)}`); });
