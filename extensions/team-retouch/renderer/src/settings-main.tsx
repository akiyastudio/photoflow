...
const SettingsRoot = () => {
  ...
  if (!controllerRef.current) controllerRef.current = createTeamSettingsController({ read: () => rpc<Json>('team.settings.get.v1'), merge: patch => rpc<Json>('team.settings.update.v1', patch), notice: setNotice });
  ...
  return <main className="min-h-screen bg-slate-50 px-8 py-10 lg:px-12"><div className="mx-auto w-full max-w-6xl"><header className="mb-8 flex items-center gap-3"><span className="pf-icon-tile p-2"><Settings size={20}/></span><div><h1 className="text-2xl font-bold text-slate-900">团片协作</h1><p className="mt-1 text-xs text-slate-500">处理偏好与团片协作专属识别环境</p></div></header>{settingsState.settings && settingsState.loaded ? <TeamSettingsContent value={settingsState.settings} patch={controller.patch} notice={setNotice}/> : <section className="team-card pf-card flex min-h-52 flex-col items-center justify-center gap-3 p-6 text-center"><p className="text-sm font-bold text-slate-700">{settingsState.loading ? '正在读取团片设置…' : '无法读取团片设置'}</p>{settingsState.error && <p role="alert" className="text-xs text-red-600">{settingsState.error}</p>}{!settingsState.loading && <button type="button" className="dialog-secondary" onClick={() => void controller.refresh()}>重试</button>}</section>}{notice && <div role="status" className="fixed bottom-5 right-5 rounded-lg bg-slate-900 px-4 py-3 text-sm text-white shadow-lg">{notice}</div>}</div></main>;
};
