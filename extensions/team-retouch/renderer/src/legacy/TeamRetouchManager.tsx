...
const useOriginalPreview = (filePath: string | undefined, cacheConfig: AppConfig['mediaCache'], enabled = true) => {
  ...
  useEffect(() => {
    ...
    setUrl('');
    if (!filePath || !enabled) return () => { active = false; };
    void legacyApi.getMediaOriginal(filePath, originalPreviewKind(filePath), cacheConfig).then(result => {
    ...
    {loading ? <div className="flex min-h-64 items-center justify-center gap-2 text-slate-500"><Loader2 className="animate-spin"/>正在读取人物数据…</div> : <div className="team-photo-layout grid grid-cols-[minmax(320px,.9fr)_minmax(440px,1.1fr)]">
      ...
      <section className="p-5">{!tasks.length ? <div className="flex min-h-96 flex-col items-center justify-center rounded-xl border border-dashed border-slate-300 bg-white p-8 text-center"><ScanFace size={34} className="text-violet-500"/><h4 className="mt-3 font-bold text-slate-800">识别人物并生成工作图</h4><p className="mt-2 text-sm text-slate-500">此步骤只识别、裁图和标记人物。</p><button onClick={() => void detect()} className="dialog-primary mt-4">开始识别</button></div> : <div className="grid gap-4 xl:grid-cols-2">{tasks.map((task, taskIndex) => {
        ...
        return <article key={task.id} className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm"><header className="flex items-start gap-3"><span className={`rounded-full p-2 ${task.needsReview ? 'bg-amber-50 text-amber-600' : taskFullyMarked ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-500'}`}>{task.needsReview ? <AlertTriangle size={17}/> : <UserRound size={17}/>}</span><div className="min-w-0 flex-1"><h4 className="text-sm font-bold text-slate-800">工作图 {taskIndex + 1}</h4><p className="mt-1 truncate text-xs text-slate-500" title={names.join('、')}>{taskMarkedCount ? `人物标记 ${taskMarkedCount}/${taskMembers.length}${names.length ? ` · ${names.join('、')}` : ''}` : `${taskMembers.length} 个人物尚未标记`}</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${task.needsReview ? 'bg-amber-50 text-amber-600' : taskFullyMarked ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-500'}`}>{task.needsReview ? '建议检查' : taskFullyMarked ? '已标记' : '未标记'}</span></header>
          ...
          <dl className="mt-3 grid grid-cols-2 gap-1 rounded-lg border border-slate-100 bg-white p-2 text-[10px] text-slate-500" data-working-image-metrics><div><dt className="inline font-bold">工作图尺寸：</dt><dd className="inline">{metrics.width && metrics.height ? `${metrics.width} × ${metrics.height} px` : '未知 · 需核对'}</dd></div><div><dt className="inline font-bold">原图占比：</dt><dd className="inline">{metrics.areaRatio === undefined ? '未知 · 需核对' : `${Math.round(metrics.areaRatio * 100)}%`}</dd></div><div><dt className="inline font-bold">裁剪范围：</dt><dd className="inline">{metrics.sourceWidth && metrics.sourceHeight ? metrics.entire ? '整幅' : '局部' : '未知 · 需核对'}</dd></div><div><dt className="inline font-bold">尺寸限制：</dt><dd className={`inline ${metrics.over4000 ? 'font-bold text-amber-700' : ''}`}>{metrics.width && metrics.height ? metrics.over4000 ? '超过 4000' : '未超过 4000' : '未知 · 需核对'}</dd></div><div className="col-span-2"><dt className="inline font-bold">识别后端：</dt><dd className="inline">{task.detectionBackend || task.backend || task.engine ? metrics.backend : '未知 · 需核对'}{metrics.fallbackReason ? ` · 已降级：${metrics.fallbackReason}` : ''}</dd></div></dl>
...
export const TeamRetouchManager = (props: Props) => {
  ...
  return <TeamRetouchWorkspace {...props} componentStatus={freshComponentStatus} advancedStatusLoading={advancedStatusLoading} advancedStatusError={advancedStatusError} onRetryAdvancedStatus={() => void refreshAdvancedStatus()}/>;
};
