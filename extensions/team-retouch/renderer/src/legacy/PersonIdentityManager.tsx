...
const SubjectThumb = memo(({ subject, cacheConfig, interactive = true, sourcePath }: { subject: Subject; cacheConfig: AppConfig['mediaCache']; interactive?: boolean; sourcePath?: string }) => {
  ...
  }, [imagePath, cacheConfig.directory, cacheConfig.maxSizeGB]);
...
const WorkflowReturnReviewDialog = ({ result, cacheConfig, busy, onClose, onConfirm, onIgnore }: {
  result: TeamPatchReturnBatchResult;
  cacheConfig: AppConfig['mediaCache'];
  busy: string;
  onClose: () => void;
  onConfirm: (match: TeamPatchReturnMatch, candidate: ReturnCandidate) => void;
  onIgnore: (match: TeamPatchReturnMatch) => void;
}) => {
  ...
  return <div role="dialog" aria-modal="true" aria-label="批量返图图片确认" className="fixed inset-0 z-[460] flex items-center justify-center bg-slate-950/70 p-4">
    <div className="flex max-h-[94vh] w-full max-w-[1500px] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
      ...
      {result.success && result.matches.length > 0 ? <div className="grid min-h-0 flex-1 grid-cols-[330px_minmax(0,1fr)]">
        ...
        <section className="flex min-h-0 min-w-0 flex-col bg-slate-950 text-white">{activeMatch ? <><div className="flex flex-wrap items-center gap-2 border-b border-white/10 bg-slate-900 px-4 py-2 text-[11px]" data-return-evidence><span className={`rounded-full px-2.5 py-1 font-bold ${matchAssessment.needsManualMatch ? 'bg-amber-500/20 text-amber-300' : 'bg-blue-500/20 text-blue-200'}`}>{matchAssessment.label}{matchAssessment.score ? ` · ${Math.round(matchAssessment.score * 100)}%` : ''}</span><span className={`rounded-full px-2.5 py-1 font-bold ${modificationAssessment.suspicious || !modificationAssessment.known ? 'bg-amber-500/20 text-amber-300' : 'bg-emerald-500/20 text-emerald-300'}`}>{modificationAssessment.label}</span>{!modificationAssessment.known && <span className="text-slate-400">旧数据无 editEvidence，必须看图核对</span>}</div><ImageComparisonView
          left={{ label: `任务图 · ${activeCandidate?.photoName || '暂无候选'}`, content: activeCandidate?.patchPath ? <ReturnImage filePath={activeCandidate.patchPath} cacheConfig={cacheConfig} eager className="absolute inset-0 h-full w-full"/> : <div className="absolute inset-0 flex items-center justify-center text-xs text-amber-300">没有可对比的候选任务</div> }}
          right={{ label: `返图 · ${activeMatch.sourceName}`, content: <ReturnImage filePath={activeMatch.mediaPath || activeMatch.path} cacheConfig={cacheConfig} eager className="absolute inset-0 h-full w-full"/> }}
          mode={mode}
...
export const PersonIdentityManager = ({ workspacePath, project, cacheConfig, activeStep, onStepChange, stageSummaries, onBlockedStage, onClose, onNotice, onProjectChanged, onBusyChange }: Props) => {
  ...
  const savePreferredIdentityOrder = async (identityOrder: string[], requestedSameWeekIdentityIds: string[], successMessage: string) => {
    ...
    const result = await legacyApi.saveTeamWorkflowSettings(workspacePath, {
  ...
  const addPreferredIdentity = async (identityId: string) => {
    ...
    await savePreferredIdentityOrder([...preferredIdentityOrder, identityId], sameWeekIdentityIds, `已将“${identityName}”加入优先顺序。请重新生成协作流程。`);
  };
  const removePreferredIdentity = async (identityId: string) => {
    ...
    await savePreferredIdentityOrder(nextOrder, sameWeekIdentityIds.filter(currentId => currentId !== identityId), nextOrder.length ? '优先顺序已更新。请重新生成协作流程。' : '已恢复自动排期。请重新生成协作流程。');
  };
  ...
  const toggleSameWeekIdentity = async (identityId: string) => {
    ...
    await savePreferredIdentityOrder(
      ...
      joinsPrevious ? '已改为下一周开始。请重新生成协作流程。' : '已改为同周开始。请重新生成协作流程。',
    );
  };
  const toggleComplete = async (item: WorkflowItem) => {
    ...
    const result = await legacyApi.completeTeamIdentity(workspacePath, {
  ...
  const removeUpload = async (item: WorkflowItem) => {
    ...
    const result = await legacyApi.removeTeamPatchUpload(workspacePath, {
      photoId: item.photo.photoId,
      taskId: item.task.id,
      personIndex: item.personIndex,
      projectName: project.name,
  ...
  const stageDescription = activeStep === 'assignment' ? '阶段 2 · 自动排期为默认；接收人、顺序和高级同周选项在此确认。' : activeStep === 'relay' ? '阶段 3 · 按接力链分发工作图并接收返图。' : '阶段 4 · 同时核对匹配度与修改有效性，再合并输出。';

  return <div className="fixed inset-x-0 bottom-0 top-10 z-[315] flex flex-col bg-slate-50">
    <header className="team-workflow-header flex min-h-16 flex-wrap items-center gap-3 border-b border-slate-200 bg-white px-5 py-3"><span className="rounded-xl bg-violet-50 p-2 text-violet-600"><UsersRound size={20}/></span><div><h2 className="font-bold text-slate-900">团片协作</h2><p className="mt-0.5 text-xs text-slate-500">{stageDescription}</p></div><TeamRetouchSteps value={activeStep} onChange={onStepChange} summaries={stageSummaries} onBlocked={onBlockedStage}/><button type="button" onClick={onClose} title="团片协作设置" aria-label="团片协作设置" className="ml-auto rounded-md p-2 text-slate-500 hover:bg-slate-100"><Settings size={20}/></button></header>
    {!loading && Boolean(workspace.photos.length) && <>
      <div className="flex min-h-14 flex-wrap items-center gap-3 border-b border-slate-200 bg-white px-5 py-2.5">
        <span className="text-xs font-bold text-slate-700">{activeStep === 'assignment' ? '任务分配' : activeStep === 'relay' ? '接力进度' : '审核输出'}</span><span className="rounded-full bg-violet-50 px-2.5 py-1 text-xs font-bold text-violet-700">{weeks.length} 周</span><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-600">{personWeekCount} 个批次</span><span className="rounded-full bg-slate-100 px-2.5 py-1 text-xs text-slate-600">{splitIdentityCount} 人跨周</span>{Boolean(workspace.missingReturnCount) && <span title="返图文件已被外部删除或移动；恢复原路径后会自动重新连接" className="rounded-full bg-red-50 px-2.5 py-1 text-xs font-bold text-red-700">返图缺失 {workspace.missingReturnCount}</span>}
        {activeStep === 'assignment' && <><span className="text-xs text-slate-500">默认自动排期 · {workspace.workflowGenerated && !workspace.workflowNeedsRegeneration ? '已生成' : '待生成'}</span><button disabled={!workflowGroups.size || Boolean(busy) || workflowGenerating} onClick={() => void generateWorkflow()} className="dialog-primary ml-auto inline-flex items-center gap-2">{workflowGenerating ? <Loader2 size={15} className="animate-spin"/> : <FolderOutput size={15}/>} {workflowGenerating ? `生成中 ${Math.round(workflowGeneration.progress)}%` : workspace.workflowGenerated ? '按当前分配重新生成' : '确认分配并生成流程'}</button></>}
      ...
      {activeStep === 'assignment' && <details className="border-b border-violet-100 bg-violet-50/70 px-5 py-2"><summary className="cursor-pointer text-xs font-bold text-violet-800">高级排期 · 拖拽顺序与同周选项（默认收起）</summary><div className="mt-2 flex min-h-10 flex-wrap items-center gap-2">
        <label title={workflowOrderLocked ? '已有任务返图或完成，开工顺序已锁定' : '未加入高级队列的人物继续自动排期'} className="inline-flex items-center gap-2 rounded-md border border-violet-200 bg-white px-2.5 py-1.5 text-xs font-bold text-violet-700"><span>优先队列</span><select value="" disabled={Boolean(busy) || workflowOrderLocked || !availableWorkflowIdentities.length} onChange={event => void addPreferredIdentity(event.target.value)} className="max-w-44 bg-transparent text-xs font-medium outline-none disabled:text-slate-400"><option value="">{preferredIdentityOrder.length ? `已排序 ${preferredIdentityOrder.length} 人 · 添加` : '添加人物'}</option>{availableWorkflowIdentities.map(identity => <option key={identity.id} value={identity.id}>{identity.name}</option>)}</select></label>
      {!!preferredWorkflowIdentities.length && <div className="contents">
      ...
      </div>} {!preferredWorkflowIdentities.length && <span className="text-[11px] text-violet-600">当前全部使用自动排期。</span>}</div></details>}
    </>}
