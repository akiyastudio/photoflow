...
export const TeamRetouchSteps = ({ value, onChange, disabled = false, summaries, onBlocked }: Props) => {
  ...
  <nav aria-label="团片协作任务" data-stage-layout="tabs" role="tablist" className="team-stage-nav ml-3 grid h-full min-w-0 grid-cols-4 overflow-x-auto text-xs font-bold">
    {WORKFLOW_STAGES.map((step, stepIndex) => { const summary = byId.get(step.id); const selected = value === step.id; const complete = Boolean(summary?.complete); const progress = !complete && progressIndex === stepIndex; const locked = Boolean(summary?.blockedReason); const tone = complete ? 'complete' : progress ? 'progress' : 'other'; return <button
      key={step.id}
      type="button"
      role="tab"
      disabled={disabled}
      ...
      onClick={() => locked ? onBlocked?.(summary?.blockedReason || '') : onChange(step.id)}
