...
export const TeamRetouchSteps = ({ value, onChange, disabled = false, summaries, onBlocked }: Props) => {
  ...
  return (
  <nav aria-label="团片协作四阶段" data-stage-layout="responsive" className="team-stage-nav ml-5 flex rounded-lg bg-slate-100 p-1 text-xs font-bold">
    {WORKFLOW_STAGES.map(step => { const summary = byId.get(step.id); const blocked = Boolean(summary?.blockedReason); return <button
      ...
      className={`team-stage-button inline-flex min-w-0 items-center gap-1.5 rounded-md px-3 py-2 transition disabled:opacity-50 ${value === step.id ? 'bg-white text-blue-600 shadow-sm' : summary?.complete ? 'text-emerald-700 hover:text-emerald-900' : blocked ? 'cursor-not-allowed text-slate-400' : 'text-slate-500 hover:text-slate-800'}`}
    >
