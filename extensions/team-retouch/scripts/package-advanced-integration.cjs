...
const { candidateRoot, advancedPackageVersion } = require('./package-output-paths.cjs');
const root = path.resolve(__dirname, '..');
const manifest = JSON.parse(fs.readFileSync(path.join(root, 'component.template.json'), 'utf8'));
validateInputLock(root, path.join(root, 'advanced/build-input-lock.json'), { componentVersion: advancedPackageVersion(manifest), advancedRuntimeApiVersion: manifest.advancedRuntime.apiVersion });
...
const advancedName = `PhotoFlow-team-retouch-advanced-${advancedPackageVersion(manifest)}-win32-x64.zip`;
