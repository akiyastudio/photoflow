...
The advanced backend uses only the `PhotoFlowNative` WSL distribution. Its failure still falls back to the basic RTMDet path. End-user lifecycle scripts automatically locate the declared standalone offline runtime ZIP in `%LOCALAPPDATA%/PhotoFlow/components/team-retouch/advanced/packages`, verify its SHA-256 against the plugin manifest and its pinned runtime version/API, and install it under the separate Host-controlled component data root. The package directory is stable across plugin updates; detection needs only the installed runtime. Legacy embedded archives remain supported. Installation never performs a network build.
