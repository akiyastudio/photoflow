...
const receiptFor = (commitId, output) => ({ apiVersion: 7, commitId, outputs: [output] });
...
const capability = async frame => {
  ...
  if (frame.method === 'component.settings') return { apiVersion: 7, revision: 1, settings: { language: 'zh', model: 'tiny', device: 'cpu', computeType: 'int8', beamSize: 2, vadFilter: true, simplifyChinese: true, cpuFallback: true } };
  if (frame.method === 'project.media.page') {
    ...
    return { apiVersion: 7, items, page: { hasMore: false, cursor: null } };
  }
  if (frame.method === 'project.files.page') {
    ...
    return { apiVersion: 7, items, page: { hasMore: false, cursor: null, pageSize: 200, truncated: false } };
  }
  if (frame.method === 'project.media.variants') { const source = path.join(projectRoot, payload.relativePath); if (!fs.existsSync(source)) throw new Error('media missing'); const token = `test-input:${crypto.randomUUID()}`; tokens.set(token, source); return { apiVersion: 7, input: { token, expiresAt: Date.now() + 60_000 }, variants: {} }; }
  ...
  if (frame.method === 'tasks') { const key = `${projectId}:${payload.operationId}`; const state = taskState.get(key) || { cancelled: false, actions: [] }; state.actions.push(payload.action); if (payload.action === 'cancel') state.cancelled = true; if (['start', 'resume'].includes(payload.action)) state.cancelled = false; taskState.set(key, state); return { apiVersion: 7, task: { state: payload.action }, cancelled: state.cancelled, checkpoint: payload.checkpoint || {} }; }
  if (frame.method === 'component.events') return { apiVersion: 7, emitted: true };
  if (frame.method === 'project.output') {
    if (payload.action === 'stage') { const stageId = crypto.randomUUID(); const privatePath = path.join(privateRoot, 'stages', stageId); fs.mkdirSync(privatePath, { recursive: true }); stages.set(stageId, { privatePath }); return { apiVersion: 7, stageId, privatePath, expiresAt: Date.now() + 60_000 }; }
