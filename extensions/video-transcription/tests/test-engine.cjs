...
(async () => { try {
  ...
  const { runEngine, runtimeStatus } = require('../service.cjs'); const savedExecutable = process.env.PHOTOFLOW_TRANSCRIBER_EXECUTABLE; const savedPrefix = process.env.PHOTOFLOW_TRANSCRIBER_ARGS_PREFIX; const savedProcessLog = process.env.PHOTOFLOW_TRANSCRIBER_PROCESS_LOG;
  ...
  try {
    ...
    await assert.rejects(runWithDeadline(runEngine({ row: { id: 'spawn-error', operationId: 'spawn-error', privatePath: inputPath }, settings: {}, outputPath: path.join(root, 'missing.srt'), signal: new AbortController().signal, onProgress: async () => {} })), /无法启动|ENOENT|运行时/);
    ...
    await assert.rejects(runWithDeadline(runEngine({ row: { id: 'progress-error', operationId: 'progress-error', privatePath: inputPath }, settings: {}, outputPath: path.join(root, 'progress-error.srt'), signal: new AbortController().signal, onProgress: async () => { throw new Error('simulated progress failure'); } })), /simulated progress failure/);
    const diagnosticLog = path.join(root, 'diagnostic-processes.txt'); process.env.PHOTOFLOW_TRANSCRIBER_ARGS_PREFIX = JSON.stringify([path.join(__dirname, 'fake-diagnostic.cjs')]); process.env.PHOTOFLOW_TRANSCRIBER_PROCESS_LOG = diagnosticLog;
    const statuses = await Promise.all([runtimeStatus(), runtimeStatus(), runtimeStatus()]); assert(statuses.every(status => status.ready && status.packaged), 'concurrent callers share a successful runtime diagnostic'); assert.equal(fs.readFileSync(diagnosticLog, 'utf8').trim().split(/\r?\n/).length, 1, 'concurrent runtime checks spawn only one diagnostic process');
  } finally { if (savedExecutable === undefined) delete process.env.PHOTOFLOW_TRANSCRIBER_EXECUTABLE; else process.env.PHOTOFLOW_TRANSCRIBER_EXECUTABLE = savedExecutable; if (savedPrefix === undefined) delete process.env.PHOTOFLOW_TRANSCRIBER_ARGS_PREFIX; else process.env.PHOTOFLOW_TRANSCRIBER_ARGS_PREFIX = savedPrefix; if (savedProcessLog === undefined) delete process.env.PHOTOFLOW_TRANSCRIBER_PROCESS_LOG; else process.env.PHOTOFLOW_TRANSCRIBER_PROCESS_LOG = savedProcessLog; }
