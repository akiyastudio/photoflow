...
const catalog = listModels(); assert.equal(catalog.placement, 'models/<model-id>'); assert.equal(catalog.directory, path.join(root, 'models')); assert.equal(catalog.downloadPolicy, 'manual-only'); assert(catalog.models.some(item => item.id === 'large-v3' && item.installed));
...
const packageScript = fs.readFileSync(path.join(root, 'scripts', 'package-component.cjs'), 'utf8'); assert(packageScript.includes("path.join(root, 'dist', 'runtime'")); assert(packageScript.includes("Release packages must not contain speech-recognition models")); assert(!packageScript.includes("--model-root")); assert(!/const files\s*=\s*\[[^\]]*models/s.test(packageScript), 'release package inputs always exclude plugin-local models');
const releaseScript = fs.readFileSync(path.join(root, 'scripts', 'package-release.cjs'), 'utf8'); assert(releaseScript.includes("run('build-runtime.cjs')") && releaseScript.includes("run('package-component.cjs'"), 'every release package builds the self-contained runtime first');
console.log('video-transcription self-contained release runtime, explicit model directory, and safe model discovery tests passed');
