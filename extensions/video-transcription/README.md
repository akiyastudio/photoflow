...
普通 Windows x64 打包生成 `PhotoFlow-video-transcription-26.8.29.9-win32-x64.zip` thin package，并保留清晰的缺失运行时诊断。打包脚本支持 `--runtime` 和 `--model-root`；正式离线包可指定已审计的 PyInstaller 产物及模型：
