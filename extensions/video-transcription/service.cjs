  ...
  try {
    ...
    for (const file of rows) {
      ...
      try {
        ...
        const result = await engine.transcribe(file, operation.settings, outputPath, async (progress, message = '') => { if (progress !== null) file.progress = progress; const completed = operation.files.filter(value => ['completed', 'failed'].includes(value.state)).length; const report = await callHost(parentId, 'tasks', { action: 'report', operationId, progress: ((completed + (Number(progress) || 0) / 100) / Math.max(1, operation.files.length)) * 100, phase: 'transcribing', message: message || `正在识别 ${file.displayName}`, checkpoint: { nextOrdinal: file.ordinal, currentFileId: file.id, total: operation.files.length } }); if (report.cancelled) control.abort(); });
