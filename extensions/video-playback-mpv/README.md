...
组件优先使用 `gpu-next` 视频输出，并在初始化不可用时回退到 `gpu`。libplacebo 必须启用 shaderc；构建和宿主打包入口会检查其 DLL 依赖，防止将缺少着色器编译器的运行库打包；两者均使用 D3D11、`hwdec=auto-safe` 和预读缓存。硬件不支持相机的 H.265 10-bit 4:2:2 时，libmpv 自身会回退到 CPU 解码。时间戳修复只在播放错误后的有界恢复阶段启用，不覆盖正常相机文件的原始时间戳。

## 从源码一键构建发布包

在照片流开发版启动前，组件会自动运行 `npm run prepare:dev`，使用本组件已有且匹配锁定版本的运行时生成开发用 EXE 和 DLL。输入和输出未变化时会直接复用；这个步骤不生成安装包。也可以在组件目录单独执行该命令。
...
```text
../../artifacts/installers/base/PhotoFlow-video-playback-mpv-26.8.31-win32-x64.zip
```
