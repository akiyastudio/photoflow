...
assert.equal(manifest.version, pkg.version, 'component manifest and npm package versions must stay in sync');
assert.match(manifest.version, /^\d{2}\.\d{1,2}\.\d{1,2}$/, 'release version must use the YY.M.DD contract');
const [year, month, day] = manifest.version.split('.').map(Number);
const releaseDate = new Date(Date.UTC(2000 + year, month - 1, day));
assert.equal(releaseDate.getUTCFullYear(), 2000 + year, 'release version year must form a valid date');
assert.equal(releaseDate.getUTCMonth(), month - 1, 'release version month must form a valid date');
assert.equal(releaseDate.getUTCDate(), day, 'release version day must form a valid date');
assert.equal(manifest.name, '高级视频解码');
const backend = manifest.runtimeContributions.find(item => item.type === 'media.playbackBackend');
assert.equal(backend.displayName, '高级视频解码（libmpv）');
...
const releaseSource = fs.readFileSync(path.join(root, 'scripts', 'build-release.cjs'), 'utf8');
assert(releaseSource.includes("'artifacts', 'installers', 'media-runtime', 'libmpv-lgpl-windows-x64'") && !releaseSource.includes('PHOTOFLOW_MPV_OUTPUT_ROOT'), 'release build must use the runtime builder safe output root without a Windows-path override');
const runtimeManifestSource = fs.readFileSync(path.join(root, 'media-runtime', 'create-mpv-manifest.cjs'), 'utf8');
assert(runtimeManifestSource.includes("require('../scripts/vendor/runtime-policy.cjs')"), 'runtime manifest generation must use the standalone project policy module');
const standaloneCopySource = fs.readFileSync(path.join(root, 'tests', 'test-standalone-copy.cjs'), 'utf8');
for (const generatedDirectory of ["'.cache'", "'artifacts'", "'dist'", "'vendor'"]) assert(standaloneCopySource.includes(generatedDirectory), `standalone copy must exclude ${generatedDirectory}`);
