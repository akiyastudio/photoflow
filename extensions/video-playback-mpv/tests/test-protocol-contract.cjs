...
assert(source.includes('SetOption("vo", probeOnly ? "null" : "gpu-next,gpu")'),'gpu-next must be preferred after verifying shaderc support');
assert.equal((source.match(/fflags=\+genpts\+igndts/g)||[]).length,1,'timestamp repair must not be a global playback option');
...
assert(source.includes('--timeline-request') && source.includes('ExtractTimelineFrames') && source.includes('data:image/jpeg;base64,'), 'timeline frame extraction must be owned by the libmpv playback component');
