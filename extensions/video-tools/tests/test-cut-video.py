...
def main():
    with TemporaryDirectory() as temporary_directory:
        ...
        development_archive = root / "extensions" / "video-tools" / "media-runtime" / "vendor" / "windows-x64" / "ffmpeg-runtime-windows-x64.zip"
        ...
        with mock.patch.object(ffmpeg_utils, "__file__", str(fake_module_path)), \
            ...
            assert ffmpeg_utils._ffmpeg_archive_candidates() == [
                str(fake_module_path.parent / "ffmpeg.zip"),
                str(fake_module_path.parent.parent / "ffmpeg.zip"),
                str(development_archive),
            ]
        ...
        packaged_archive = packaged_runtime_root / "ffmpeg.zip"
        ...
        with mock.patch.object(ffmpeg_utils.sys, "frozen", True, create=True), \
            ...
            assert ffmpeg_utils._ffmpeg_archive_candidates() == [
                ...
                str(packaged_worker.parent.parent / "ffmpeg.zip"),
            ]
    ...
    utf8_probe = subprocess.run(
        [
            sys.executable,
            "-c",
            f"import sys; sys.path.insert(0, {str(ROOT / 'runtime')!r}); import cut_video; cut_video.emit('log', '视频大小 4.86 GB，预计分为 2 段')",
        ],
        capture_output=True,
